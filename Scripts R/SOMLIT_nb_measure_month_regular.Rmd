...
SOMLIT_carbo_chemistry_missing_value$year <- as.numeric(SOMLIT_carbo_chemistry_missing_value$year)
SOMLIT_carbo_chemistry_missing_value$month <- as.numeric(SOMLIT_carbo_chemistry_missing_value$month)
...
SOMLIT_carbo_chemistry_missing_value <- SOMLIT_carbo_chemistry_missing_value %>% 
  mutate(month = case_when(month == 1 ~ paste("0", month, sep=""), TRUE ~ month)) #a continuer

mutate(month = paste(month["1":"9"], paste("0")))


##

test2 <- SOMLIT_carbo_chemistry_missing_value %>% 
  unite(col=datetime, c("year", "month"), sep="-") %>% 
  mutate(Year.Month = as.POSIXct(paste(datetime, "-01", sep = ""), format = "%Y-%m-%d")) 


#number of measure by month by year :
Nb_by_month_final <- summarise(group_by(test2, Year.Month), NB_OBS_M=n()) 
#must be 8 or 10
