...
atmospheric_co2_MONTE_CIMONE <- atmospheric_co2_MONTE_CIMONE[-c(1:994),]
...
plot_pCO2_water_vs_air <- DATA %>% 
  ...
  scale_y_continuous(name="pCO2 (ppm)")
...
```{r calcul flux wanninkhof 2014}

#Formula : F = 7.7 * U^2 * delta_pCO2
#unit : mol/m2/y, ici mol/m2/d car moyenne wind speed par jour
...
#F en mol/m2/day
Flux_wan_2014 <- 0.00077 * DATA$wind_mean_daily^2 * DATA$delta_pCO2

#integration au dataset
DATA <- DATA %>% mutate(Flux_wan_2014 = 0.00077 * wind_mean_daily * delta_pCO2)


```


```{r outliers flux wanninkhof 2014}
#plot visualiser les outliers
DATA %>% 
  ggplot() +
  aes(x=sampling_date, y=Flux_wan_2014) +
  geom_boxplot() + 
  xlab(label = "") +
  ylab(label = "") +
...
DATA %>% 
  ...
  geom_line(size=0.7) + 
  scale_x_datetime(name="") +
  scale_y_continuous(name="Flux (mmol/m²/day)") +
...
```{r calcul flux formule DE CARLO + package marelac}
#delta_pCO2 :
delta_pCO2 <- DATA$pCO2_water_ppm - DATA$pCO2_air_ppm
DATA <- DATA %>% mutate(delta_pCO2 = pCO2_water_ppm - pCO2_air_ppm)

#Formula DE CARLO : test pour 25 °C
2073.1 - (125.62*25) + (3.6276 * 25^2) - (0.043219 * 25^3) #524.5531
#package marelac : test pour 25 °C
gas_schmidt(t=25, species="CO2") #524.553125

#test ok
#confirmation regression lineaire entre les 2 variables
Sc_test <- gas_schmidt(t=DATA$temperature, species="CO2")
...
##### Essai calcul F avec alpha = 2.912\*10\^-2 pour une temperature de 24°C et une salinite de 35
