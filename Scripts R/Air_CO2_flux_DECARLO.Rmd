...
plot_pCO2_water_vs_air <- DATA %>% 
  ggplot() + 
  ggtitle("Water and atmosphere pCO2 according to time") +
  geom_point(aes(x=datetime, y=pCO2_water_uatm), size=1, col='#357AB7') + 
  geom_point(aes(x=datetime, y=pCO2_air_ppm), size=1, col='#EED153') + 
...
#delta_pCO2 :
delta_pCO2 <- DATA$pCO2_water_uatm - DATA$pCO2_air_ppm
DATA <- DATA %>% mutate(delta_pCO2 = pCO2_water_uatm - pCO2_air_ppm)
