...
SOMLIT_carbo_chemistry_surf_test <- SOMLIT_carbo_chemistry_surf %>% 
  filter(sampling_date < 	"2016-01-05") %>% 
  mutate(N=n(sampling_date))
  

  
tmp <- ungroup(SOMLIT_carbo_chemistry_surf) %>% 
  group_by(monthd) %>%
  summarise(
...
# Calculate anomalies
ano_month0m <- left_join(ungroup(SOMLIT_carbo_chemistry_surf), tmp, by = "monthd") %>%
  mutate(
