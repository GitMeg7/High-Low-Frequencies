...
## Data EOL surface : temperature (°C), salinity and oxygen (µmol kg^-1^)  
  

```{r visualisation data EOL, echo=TRUE, warning=FALSE, message=FALSE, fig.width=9,fig.height=5}
...
plot_EOL_RAW_AOU <- EOL_RAW %>% 
  ggplot() +
  aes(x=datetime, y=neg_AOU) +
  geom_line() +
  scale_x_date(name="") +
  scale_y_continuous(name=expression(paste("-AOU (μmol ", kg^-1, ")")))

 # plot_grid(plot_EOL_RAW_temp, plot_ano_temp_EOL_raw,
 #           plot_EOL_RAW_sal, plot_ano_sal_EOL_raw, 
 #           plot_EOL_RAW_AOU, plot_ano_AOU_EOL_raw,
 #           align='vh', ncol=2)
...
monthly_means <- ungroup(EOL_RAW) %>% 
  ...
  summarise(
    ...
    SD_neg_AOU = sd(neg_AOU, na.rm= TRUE))
...
var_list <- c("Salinity_ano", "Temperature_ano", "AOU_ano", "neg_AOU_ano")
...
plot_ano_temp_EOL_raw <- ggplot(data = anomalies, aes(x = datetime, y = Temperature_ano), na.rm=TRUE) +
  scale_x_date(date_minor_breaks="2 years", labels = NULL) +
...
```{r plot anomalies + trends - salinity, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8,fig.height=6}
...
**Plot of anomalies : -AOU (2018-2022) - EOL**  
  
```{r plot anomalies + trends - AOU, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8,fig.height=6}

plot_ano_AOU_EOL_raw <- ggplot(data = anomalies, aes(x = datetime, y = neg_AOU_ano), na.rm=TRUE) +
  scale_x_date(date_minor_breaks="2 years", labels = date_format("%Y")) +
  geom_point(colour="#22427C", na.rm=TRUE, size=0.7) + 
  geom_smooth(data=anomalies[c(4667 : 47184),], method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="",x="", y=expression(paste("-AOU (μmol ", kg^-1, ")")))


plot_EOL_RAW_AOU
plot_ano_AOU_EOL_raw
...
### Study of minima and maxima (daily) of temperature :  
...
#### Part 3 : Trend analysis by residuals : hourly dataset jan.2018 - dec.2022  
  
  

```{r importation Data EOL 3 - hourly, echo=FALSE, warning=FALSE, message=FALSE}

EOL_daily_3 <- read_delim("../../Data/data_EOL/EOL_18-23_hourly.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE) %>% 
  rename(temperature=temp_eol_c, salinity=sal_eol_psu) %>% 
  mutate(datetime = as.POSIXct(datetime)) %>% 
