...
```{r calcul monthly means - temperature & salinity, echo=TRUE, warning=FALSE, message=FALSE, fig.width=8,fig.height=10}
...
monthly_means_rapport <- monthly_means %>% 
  mutate(`T (°C)` = paste(round(Temperature_month, digits=2), "±", round(SD_temp, digits=2)),
         ...
         `T (°C)` = paste(round(Temperature_month, digits=2), "±", round(SD_temp, digits=2)),
         S = paste(round(Salinity_month, digits=2), "±", round(SD_sal, digits=2)))

monthly_means_rapport <- monthly_means_rapport %>% 
  dplyr::select(Month, `T (°C)`, S, SD_sal, SD_temp, Salinity_month, Temperature_month) %>% 
  rename(`Mois  ` = Month)
        
        
kable(monthly_means_rapport, caption="Monthly means at EOL (surface)", format = "pipe", align='l')
...
```{r daily variation EOL 3, echo=FALSE, warning=FALSE, message=FALSE}

EOL_daily_3 <- EOL_daily_3 %>% 
  mutate(datetime = format(datetime, format="%Y-%m-%d"))


annual_cycle_temp_eol <- EOL_daily_3 %>% 
  ggplot() +
...
annual_cycle_oxy_eol <- EOL_daily_3 %>% 
  ggplot() +
