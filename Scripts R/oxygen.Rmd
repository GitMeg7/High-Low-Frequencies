...
plot_ano_AOU_surf <- ggplot(data = anomalies_AOU_surf, aes(x = datetime, y = AOU_ano), na.rm=TRUE) +
  ...
  annotate(geom="text", x=as.POSIXct("2012-03-20"), y=80, label="slope : -0.47 ± 0.065 *", color="black")
...
data_somlit_prof %>% 
  ...
  ggtitle("AOU according to temperature : Point B (2002-2022)") +
  geom_smooth(method=lm) + 
  annotate(geom="text", x=22, y=60, label="slope : -2.82 ± 0.23 *", color="black")

#summary(lm(data_somlit_prof$AOU ~ data_somlit_prof$T))
##
...
*diminution de l'AOU (µmol/kg) : *  
*slope : -0.46 ± 0.074 µmol/kg par année* 
...
plot_ano_AOU_prof <- ggplot(data = anomalies_AOU_prof, aes(x = datetime, y = AOU_ano), na.rm=TRUE) +
  ...
  annotate(geom="text", x=as.POSIXct("2012-03-20"), y=80, label="slope : -0.46 ± 0.074 *", color="black")
...
plot_AOU_ts_prof_less_season <- autoplot(AOU_ts_prof_less_season) + 
  ...
  ggtitle("AOU 50 m depth time series (without seasonality)") +
  geom_smooth(method=lm) +
  annotate(geom="text", x=2013, y=57, label="slope : -0.4136 ± 0.076 *", color="black")
...
#pour EOL : identifier les periode de manque, justifier que ces périodes ne bougent pas bcp pour interpoler ?
...
#faire daily variations des paramètres

###AOU :
#moyenne annuelle interessante
...
###Laurent
#ASTI san sebastien glyder, en cours
