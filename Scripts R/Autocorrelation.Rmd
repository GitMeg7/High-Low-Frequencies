...
## Import data :
#filter values until 2021, because no pCO2 air data for 2022
...
plot_ano_flux_wan_14 <- ggplot(data = anomalies_fluxes, aes(x = datetime, y = flux_W14_ano), na.rm=TRUE) +
  ...
  annotate(geom="text", x=as.Date("2012-03-20"), y=0.0010, label="slope : 4.81e-06 ± 1.17e-06 *", color="black")
...
Trend : decrease but increase at the end ?  
  

```{r pH_18 - point b - autocorr, echo=FALSE, warning=FALSE, message=FALSE}

#creation ts object : pH_18

#NA : creation de impute pH_18
#bcp de NA car pH_calc basé sur Pt & Sit

#arret data pH_18 : 2021-10-19:
data_somlit_surf_pH_18 <- data_somlit_surf %>%
  dplyr::filter(datetime <= "2021-10-19") %>% 
  mutate(pH_18 = na_interpolation(pH_18))



#frequency : 365.25/7 = 52.17857
#arret data pH_calc au 2022-09-28 :
ts_pH_18 <- ts(data_somlit_surf_pH_18$pH_18, start = c(2007,1), end = c(2022,39), freq = 52)
...
plot_ts_pH_18_less_season <- autoplot(ts_pH_18_less_season) + 
  ...
  ggtitle("pH(18) time series (without seasonality)") +
  geom_smooth(method=lm) +
  annotate(geom="text", x=2015, y=8.13, label="slope : -0.0021 ± 0.00014 *", color="black")
