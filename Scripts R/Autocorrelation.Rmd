...
FF <- kendallSeasonalTrendTest(impute_temp ~ month(datetime) + year(datetime), data = data_somlit_surf)
FF$estimate
...
plot_ano_pCO2bio <- ggplot(data = anomalies, aes(x = datetime, y = bio_pco2_ano), na.rm=TRUE) +
  ...
  annotate(geom="text", x=as.Date("2010-03-02"), y=80, label="slope : 3.56 ± 0.15 *", color="black")
...
ts_pH <- ts(data_somlit_surf$impute_pH, start = c(2007,1), end = c(2022,39), freq = 52)
...
FFpH <- kendallSeasonalTrendTest(impute_pH ~ month(datetime) + year(datetime), data = data_somlit_surf)
FFpH$estimate
