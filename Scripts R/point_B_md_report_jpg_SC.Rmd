...
if (Sys.getenv("LOGNAME") == "steevecomeau") {
  path = "data/"
}
...
cutoff_date <- "2022-01-01" #to limit regressions on complete years
...
ggreg_old <- function (fit, point_size=2) {
...
```{r Initialization Point B, message=FALSE, include=TRUE}
z <-  read_delim(
  paste0(path, "Data_DIC.csv"),
  delim = ";",
  col_names = TRUE,
  col_types = "cccdddddddddddcddddddddddddddd",
  na = c("NA", "999999", "")
  )
#H <- NULL
z <- z %>%
...
pom <- read_csv(file = paste0(path, "POC-2007-2015_PtB_1m.csv"), comment="#")
...
#First, create df with pH variables plus mean ta to calculate pH when at is still unknown
# add the mean when ta = NA
tmp <- z %>% 
  ...
  dplyr::mutate(ta_filled_mean = case_when(!is.na(ta) ~ ta,
                    TRUE ~ mean(ta, na.rm=TRUE))
  )   
# set to NA qglags = 3; calculate in situ spec pH at eol
eol <- tmp %>%
  dplyr::select(sampling_date, depth, ta_filled_mean, dic, salinity_field, temp_field, 
                pH_s_eol_temp_lab, temp_lab_eol, PO4, SIOH4) %>%
  tidyr::drop_na(sampling_date, depth, ta_filled_mean, salinity_field, temp_field, 
                pH_s_eol_temp_lab, temp_lab_eol) %>%
  #dplyr::filter(qflag_pH_s_eol != 3) %>% 
  dplyr::mutate(pH_s_eol_temp_insi = pHinsi(pH = pH_s_eol_temp_lab, ALK=ta_filled_mean,
                             Tinsi = temp_field,
                             Tlab =temp_lab_eol,
...
ptb <- tmp %>%
  ...
  dplyr::mutate(pH_s_ptb_temp_insi = pHinsi(pH=pH_s_ptb_temp_lab, ALK=ta_filled_mean,
                             Tinsi=temp_field,
                             Tlab=temp_lab_ptb,
                             Pinsi=depth/10, S=salinity_field,
                             Pt=1e-6*PO4, Sit=1e-6*SIOH4,
                             k1k2 = "l", kf = "pf", ks="d",
                             pHscale = "T", b="l10")) %>% 
  dplyr::mutate(pH_s_somlit_temp_25 = pHinsi(pH = pH_s_ptb_temp_lab, ALK = ta_filled_mean,
...
dat3 %>% # this is to get rid of NaN; ugly but it works!
  readr::write_csv(file = "output_jp/data/output.csv")
...
# join
somlit <- dplyr::left_join(dat, pH25, by = c("sampling_date", "depth")) %>% 
...
dat <- dat %>% 
  dplyr::filter(!is.na(ta)) # to eliminate when there is no at values 
                            #  pH spec was calculated using mean ta when at was not yer available)
...
#save data
write_csv(somlit, file = "output_jp/data/output_somlit.csv")
#write_csv(somlit, file = paste0(path, "output_jp/output/output_somlit.csv"))
#write_csv(dat, file="output_jp/data/output.csv")
#write_csv(somlit, file = paste0(path, "output_jp/output/output.csv"))
write_csv(pangaea, file="output_jp/data/output_pangaea.csv")
#write_csv(somlit, file = paste0(path, "output_jp/output/output_pangaea.csv"))
#write_csv(vlfrcc, file="output_jp/data/output_vlfrcc.csv")
#write_csv(somlit, file = paste0(path, "output_jp/output/output_vlfrcc.csv"))
#write_rds(dat, "output_jp/data/output.rds")
#write_csv(somlit, file = paste0(path, "output_jp/output/output.rds"))
```
...
T1p_pres <- ggplot(dat0m, aes(x = sampling_date, y = temperature), na.rm=TRUE) +
  scale_x_datetime(date_breaks="3 year", date_minor_breaks="1 year", labels = date_format("%Y")) +
  geom_point(colour="blue", na.rm=TRUE, size=size_point) + 
  labs(title=NULL,x="", y="Temperature (°C)") +
  coord_fixed() +
  Mytheme(size_labs = 8) +
  theme(plot.margin = margin(r = 0.5,
                             l = 0.5, unit = "cm"))

pH1p_pres <- ggplot(dat0m, aes(x = sampling_date, y = pH_calc), na.rm=TRUE) +
  scale_x_datetime(date_breaks="3 year", date_minor_breaks="1 year", labels = date_format("%Y")) +
  geom_point(colour="blue", na.rm=TRUE, size=size_point) + 
  labs(title=NULL,x="", y=expression(paste(pH[T]," calculated"))) +
  coord_fixed() +
...
ggsave("output_jp/figures/fig_pH1p_pres.png", pH1p_pres, width=15, height = 10, units = "cm")
ggsave("output_jp/figures/fig_T1p_pres.png", T1p_pres, width=15, height = 10, units = "cm")
...
lms <- lapply(var_list, function(x) {
  summary(lm(substitute(i ~ decimal_date(sampling_date), list(i = as.name(x))), data = dplyr::filter(ano_month50m, sampling_date < cutoff_date)))
  })
reg <- NULL
for (i in 1:length(var_list)) {
  #one loops through all anomalies
...
plt <- ggreg(xdata = dat0m$salinity, ydata = dat0m$ta, type = "OLS", title = "AT vs Sal", plot = TRUE)
ggsave(file="output_jp/figures/at_sal.png", plt, width = 18, height = 21, units = "cm")
```
![at_sal.png](output_jp/figures/at_sal.png)

The relationship between AT (in umol/kg) and salinity using mean data at 1 m is:
AT = `r round(reg_AT_S_all[4], digit=2)` + S x `r round(reg_AT_S_all[1], digit=2)` with an r2 of `r round(reg_AT_S_all[9], digit=2)`

## Daily and monthly changes in pH and T
```{r Daily and seasonal changes, include=FALSE}
# Long series
continuous_discrete <- read_csv(file = paste0(path, "continuous_discrete.csv"), 
                                col_names = TRUE,
...
```{r CO2, include=FALSE}
CO2_pr <- tbl_df(
  read_csv(file=paste0(path, "CO2_data_PlateauRosa_daily_1993-2015_R.csv"), col_names=TRUE, skip=7)
) %>%
  rename(dt=date) %>%
  mutate(year=year(dt)) %>%
...
library(lmodel2)
test= dat3[complete.cases(dat3[ , c(11,24)]),]
test50=filter(test, depth == 50)
test1=filter(test, depth == 0)

reg2=lmodel2(pH_calc~pH_s_somlit_temp_insi, data=test50, "relative","relative",nperm=99)
#reg2


pH_comp_50=ggplot(test50, aes(x=pH_s_somlit_temp_insi, y=pH_calc))+ 
  geom_point()+
  ylab("pH calculated")+
  xlab("pH measured")+
  geom_abline(intercept= -3.282352, slope = 1.4037012, col="blue", size=1.5)+
  geom_abline(intercept= 0, slope = 1, linetype="dashed")+
  annotate(geom="text", x=8.115, y=8.025, label="y = -3.28 + 1.40 x ; r2 = 0.31",
           color="blue")+
  ggtitle("50 m")

pH_comp_50=pH_comp_50 + theme(
...
pH_comp_50
#Type 2 regression
reg2_0=lmodel2(pH_calc~pH_s_somlit_temp_insi, data=test1, "relative","relative",nperm=99)
#reg2_0



pH_comp_1=ggplot(test1, aes(x=pH_s_somlit_temp_insi, y=pH_calc))+ 
  geom_point()+
  ylab("pH calculated")+
  xlab("pH measured")+
  geom_point()+
  geom_abline(intercept= -0.3088620, slope = 1.0367075, col="blue", size=1.5)+
  geom_abline(intercept= 0, slope = 1, linetype="dashed")+
  annotate(geom="text", x=8.07, y=7.925, label="y = -0.15 + 1.02 x ; r2 = 0.90",
           color="blue")+
  ggtitle("1 m")

pH_comp_1=pH_comp_1 + theme(
  plot.title = element_text(size=14, face="bold"),
  axis.title.x = element_text(size=14, face="bold"),
  axis.title.y = element_text(size=14, face="bold"),
  text = element_text(size=16)
...
#Dy graph pH Calc pH Meas 1 m
pH_Dat_1 <- dplyr::select(test1,sampling_date, pH_calc, pH_s_somlit_temp_insi)

pH_Dat_xts_1 <- as.xts(pH_Dat_1, order.by=pH_Dat_1$sampling_date)

dygraph(pH_Dat_xts_1, ylab = "", main = "1 m") %>%
  dySeries("pH_calc", drawPoints = TRUE, pointSize=3, color = "red", label = "pH Calc") %>%
 dySeries("pH_s_somlit_temp_insi", drawPoints = TRUE, pointSize=2, label = "pH measured")

#Dy graph pH Calc pH Meas 50 m
pH_Dat <- dplyr::select(test50,sampling_date, pH_calc, pH_s_somlit_temp_insi)

pH_Dat_xts <- as.xts(pH_Dat, order.by=pH_Dat$sampling_date)

dygraph(pH_Dat_xts, ylab = "", main = "50 m") %>%
  dySeries("pH_calc", drawPoints = TRUE, pointSize=3, color = "red", label = "pH Calc") %>%
  dyAxis("y", valueRange= c(8.05, 8.2))%>%
  dyAxis("y2", valueRange= c(8.05, 8.2))%>%
 dySeries(axis = "y2","pH_s_somlit_temp_insi", drawPoints = TRUE, pointSize=2, label = "pH measured")
