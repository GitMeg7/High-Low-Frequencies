...
monthly_means_0m_0722 <- ungroup(data_0722) %>% 
  ...
  summarise(
    ...
    pH_calc_month = mean(impute_pH, na.rm = TRUE),
    SD_PH = sd(impute_pH, na.rm = TRUE),
    pH18_month = mean(impute_pH_18, na.rm = TRUE),
    SD_PH18 = sd(impute_pH_18, na.rm = TRUE),
...
ano_0m_0722 <- left_join(ungroup(data_0722), monthly_means_0m_0722, by = "Month") %>%
  mutate(
    ...
    pH_calc_ano = impute_pH - pH_calc_month,
    pH18_ano = impute_pH_18 - pH18_month,
...
plot_ano_ph2 <- ggplot(data = ano_0m_0722, aes(x = datetime, y = pH18_ano), na.rm=TRUE) +
  scale_x_date(date_breaks="4 year", date_minor_breaks="1 years", labels = NULL) +
  geom_point(colour="#22427C", na.rm=TRUE, size=0.7) + 
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
...
plot_ano_ta_0722 <- ggplot(data = ano_0m_0722, aes(x = datetime, y = ta_ano), na.rm=TRUE) +
  ...
  geom_point(colour="#22427C", na.rm=TRUE, size=0.7) + 
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="",x="",y= "Total alkalinity (µmol/kg)")
...
plot_ta_0722 <- data_0722 %>% 
  ...
  geom_line(size=0.7, col="#303030") +
  scale_x_date(name="", date_breaks="4 year", date_minor_breaks="1 years", labels = NULL) +
  scale_y_continuous(name="Total alkalinity (µmol/kg)") 
#  ggtitle("Surface total alkalinity time series (2007-2022)")
...
```{r rapport, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10,fig.height=11}

plot_grid_1 <- plot_grid(plot_temp_0722, plot_ano_temp2, plot_sal_0722, plot_ano_sal2, plot_ph_0722, plot_ano_ph2, 
          plot_ta_0722, plot_ano_ta_0722, plot_pCO2_0722, plot_ano_pCO22, align='vh', ncol=2)

plot_grid_1

 # ggsave(filename = "kapsenberg_07-22.jpg", # Nommez le fichier dans lequel vous voulez enregistrer, ajoutez l'extension du format de fichier que vous voulez utiliser (ex. pdf).
 #         plot = plot_grid_1, # Fournir le nom de l'objet plot dans R
...
for(i in seq_along(var_list)){
  lm.test[[i]] <- lm(reformulate(var_list[i], "datetime"), data = ano_50m_0722)
}
...
plot_ano_ph2_50m <- ggplot(data = ano_50m_0722, aes(x = datetime, y = pH18_ano), na.rm=TRUE) +
  scale_x_date(date_breaks="4 year", date_minor_breaks="1 years", labels = NULL) +
  geom_point(colour="#22427C", na.rm=TRUE, size=0.7) + 
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="", x="", y="pH (18)")
...
plot_ano_ta_0722_50m <- ggplot(data = ano_50m_0722, aes(x = datetime, y = ta_ano), na.rm=TRUE) +
  ...
  geom_point(colour="#22427C", na.rm=TRUE, size=0.7) + 
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="",x="",y="Total alkalinity (μmol/kg)")
...
plot_ta_0722_50m <- data_0722_50m %>% 
  ...
  geom_line(size=0.7, col="#303030") +
  scale_x_date(name="", date_breaks="4 year", date_minor_breaks="1 years", labels = NULL) +
  scale_y_continuous(name="Total alkalinity (μmol/kg)") 
...
```{r rapport_50m, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10,fig.height=11}

plot_grid_2 <- plot_grid(plot_temp_0722_50m, plot_ano_temp2_50m, plot_sal_0722_50m, plot_ano_sal2_50m, 
                         plot_ph_0722_50m, plot_ano_ph2_50m, plot_ta_0722_50m, plot_ano_ta_0722_50m, 
                         plot_pCO2_0722_50m, plot_ano_pCO22_50m, align='vh', ncol=2)
