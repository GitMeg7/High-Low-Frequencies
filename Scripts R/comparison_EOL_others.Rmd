...
```{r solemio plot, echo=FALSE, warning=FALSE, message=FALSE}
...
#### Trend analysis by anomalies for SST (Bates et al., 2014)  
  
Temperature : same trend (decrease), more accentuated for EOL  
Salinity : same trend (decrease), same result for both  
...
plot_ano_EOL <- ggplot(data = anomalies_EOL_solemio, aes(x = datetime, y = Temperature_ano_eol), na.rm=TRUE) +
  ...
  annotate(geom="text", x=as.POSIXct("2019-03-16 01:00:00"), y=5, label="slope : -0.09 ± 0.007 *", color="black")
...
plot_ano_EOL_2 <- ggplot(data = anomalies_EOL_solemio, aes(x = datetime, y = Salinity_ano_eol), na.rm=TRUE) +
  ...
  annotate(geom="text", x=as.POSIXct("2019-03-16 01:00:00"), y=5, label="slope : -0.052 ± 0.002 *", color="black")
...
plot_ano_solemio <- ggplot(data = anomalies_EOL_solemio, aes(x = datetime, y = Temperature_ano_sol), na.rm=TRUE) +
  ...
  annotate(geom="text", x=as.POSIXct("2019-03-16 01:00:00"), y=5, label="slope : -0.03 ± 0.006 *", color="black")
