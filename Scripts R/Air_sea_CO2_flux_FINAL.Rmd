...
SOMLIT_pcO2_raw <- read_delim("../Data/DATA_SOMLIT_07-22_CLEAN.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)


#surface data :
SOMLIT_pcO2 <- SOMLIT_pcO2_raw %>% 
...
Somlit_interpol_50 <- Somlit_interpol_50 %>% 
  mutate(impute_pco2_w = na_seadec(pCO2_w, algorithm = "interpolation", find_frequency = T),
         ...
         impute_pH = na_seadec(pH_calc, algorithm = "interpolation", find_frequency = T),
         impute_pH_18 = na_seadec(pH_18, algorithm = "interpolation", find_frequency = T))
...
#save :
#write.table(SOMLIT_50, "DATA_SOMLIT_07-22_impute_50m.csv", sep=";", col.names = TRUE, row.names = FALSE)
...
→ Interpolation temperature & salinity :  
...
→ Interpolation pCO2 water :  
  

```{r interpolation pCO2_w, echo=FALSE, warning=FALSE, message=FALSE}

#interpolation pCO2_w :
impute_pCO2_w <- na_seadec(Somlit_interpol$pCO2_w, algorithm = "interpolation", find_frequency = F)
...
impute_ta <- na_seadec(Somlit_interpol$ta, algorithm = "interpolation", find_frequency = T)
...
→ Interpolation pH & pH(18) :  
  

```{r interpolation pH, echo=FALSE, warning=FALSE, message=FALSE}

#interpolation pH
impute_pH <- na_seadec(Somlit_interpol$pH_calc, algorithm = "interpolation", find_frequency = F)

ggplot_na_imputations(Somlit_interpol$pH_calc, impute_pH, x_axis_labels = Somlit_interpol$datetime, xlab = "", 
                      ylab="pH calc", 
                      title="Point B - pH calculated - period 2007-2022")
##

#interpolation pH(18)
impute_pH_18 <- na_seadec(Somlit_interpol$pH_18, algorithm = "interpolation", find_frequency = T)

ggplot_na_imputations(Somlit_interpol$pH_18, impute_pH_18, x_axis_labels = Somlit_interpol$datetime, xlab = "", 
                      ylab="pH (18)", 
                      title="Point B - pH (18) - period 2007-2022")
##

#merge of impute ta & CT : 
Somlit_interpol <- Somlit_interpol %>% 
  mutate(impute_pH = impute_pH,
         impute_pH_18 = impute_pH_18)
...
#join Somlit_interpol x2 to merge3 :
...
```{r annual cycle of pCO2 (2007-2021), echo=FALSE, warning=FALSE, message=FALSE, fig.width=8,fig.height=5}

annual_cycle_pco2Plot <- merge3[c(1:782),] %>% 
  ggplot() +
  ...
  theme_bw() +
  scale_y_continuous(name = "pCO2 océanique (µatm)") 
...
```{r NpCO2 & TpCO2 calculations, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10,fig.height=6}
...
#plot deltas :
pCO2bio_pCO2T_plot_long <- merge3[c(1:782),] %>% 
...
```{r atmospheric & water pCO2 plot, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10,fig.height=6}

atmos_water_pCO2_plot_long <- merge3[c(1:782),] %>% 
...
```{r calcul flux, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10,fig.height=6}

#Calcul of F
tab_flux <- merge3[c(106:782),] %>% 
...
tab_flux %>% 
  ...
  scale_x_datetime(name="", date_breaks = "2 year", date_minor_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(name="Flux (mol of C/m²/y)", labels = label_scientific(), n.breaks=8) +
...
```{r 3 methods comparison - plot, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10,fig.height=6}
...
fluxes_co2_plot <- tab_flux_long %>% 
  ...
  ggtitle("") +
  geom_line(aes(x=datetime, y=value, color= Legend), size=0.5) +
  scale_y_continuous(name="CO2 fluxes (mol/m²/h)", labels = label_scientific(), n.breaks=8) + 
  scale_x_datetime(name="", date_breaks = "2 year", date_minor_breaks = "1 year", date_labels = "%Y") +
...
```{r flux annuels, echo=FALSE, warning=FALSE, message=FALSE, fig.width=7,fig.height=5}
...
test %>% 
  ...
  scale_y_continuous(name="CO2 fluxes (mol of C/m²/h)", labels = label_scientific(), n.breaks = 10) +
  ggtitle("") 
