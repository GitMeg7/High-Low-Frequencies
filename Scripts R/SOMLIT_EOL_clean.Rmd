...
new_one <- new_one %>% 
  dplyr::select(sampling_date, depth, temperature, salinity, ta, dic, NO2, NO3, Chla, pH_s_somlit_temp_25, pH_s_somlit_temp_insi, 
...
#save :
#write.table(ph_part_result, "DATA_SOMLIT_07-22_CLEAN.csv", sep=";", col.names = TRUE, row.names = FALSE)
...
ph_part <- read_delim("../Data/DATA_SOMLIT_07-22_CLEAN.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)

ph_part_na <- ph_part %>% 
  tidyr::drop_na(salinity, temperature, ta, dic)

carb <- seacarb::carb(flag=15, var1=ph_part_na$ta*1e-6, var2=ph_part_na$dic*1e-6,
             S=ph_part_na$salinity, T=ph_part_na$temperature, P=ph_part_na$depth/10,
             Pt=1e-6*ph_part_na$Pt, Sit=1e-6*ph_part_na$Sit, 
             k1k2 = "l", kf = "pf", pHscale = "T", b="l10")

ph_calc <- data.frame(sampling_date=ph_part_na$sampling_date, depth=carb$P*10 ,pH_calc_new=carb$pH)

ph_part_result <- left_join(ph_part, ph_calc, by=c("sampling_date", "depth"))

ph_part_result <- ph_part_result %>% 
  mutate(pH_calc = pH_calc_new) %>% 
  dplyr::select(-pH_calc_new)

##

#pH18 calculation from pH_calc (fonction pHinsi) :
#pH normalized (without temperature effect)

ph_part_na <- ph_part_result %>% 
  tidyr::drop_na(salinity, temperature, Pt, Sit, ta, pH_calc)

pHat18 <- seacarb::pHinsi(pH=ph_part_na$pH_calc, ALK=ph_part_na$ta*1e-6, Tinsi=18,
                            Tlab=ph_part_na$temperature, S=ph_part_na$salinity,
                            Pt=1e-6*ph_part_na$Pt, Sit=1e-6*ph_part_na$Sit,
                            k1k2 = "l", kf = "pf", pHscale = "T")

pH_18 <- data.frame(sampling_date=ph_part_na$sampling_date, depth=ph_part_na$P*10 ,pH_18_new=pHat18)

ph_part_result <- left_join(ph_part_result, pH_18, by=c("sampling_date","depth"))

ph_part_result <- ph_part_result %>% 
  mutate(pH_18 = pH_18_new) %>% 
  dplyr::select(-pH_18_new)
...
## Creation SOMLIT oxygen (jan.2002 - dec.2022) :  
  
→ oxygen in ml/l, convert into µmol/kg


```{r SOMLIT oxygen (2002-2022), echo=FALSE, warning=FALSE, message=FALSE}

SOMLIT_oxy <- read_delim("../Data/Somlit_Extraction_hydro_oxy_2002_2022.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE) %>% 
  mutate(O = case_when(O > 10 ~ NA_real_, TRUE ~ O))
  
SOMLIT_oxy <- SOMLIT_oxy %>% 
  mutate(HEURE = as.character(HEURE),
         DATE = str_replace_all(DATE, "/", "-")) %>% 
  unite(col='datetime', c('DATE', 'HEURE'), sep=' ') %>% 
  mutate(datetime = as.POSIXct(datetime, format="%d-%m-%Y %H:%M:%S"))

SOMLIT_oxy <- SOMLIT_oxy %>% 
  mutate(T = case_when(T > 30 ~ NA_real_, TRUE ~ T),
         S = case_when(S > 39 ~ NA_real_, TRUE ~ S),
         S = case_when(S < 35 ~ NA_real_, TRUE ~ S))
...
SW_density_somlit <- rho(S = SOMLIT_oxy$S, T=SOMLIT_oxy$T, P=0)/1000
convert_o2_somlit <- 44.66/SW_density_somlit


SOMLIT_oxy <- SOMLIT_oxy %>% 
   mutate(oxy_umol_kg = O*convert_o2_somlit,
          year = format(datetime, format = "%Y"),
          month = format(datetime, format = "%m"),
          year.month = format(datetime, format = "%Y-%m"))

#delate rows with depth = 0 :
SOMLIT_oxy <- SOMLIT_oxy[-c(2051, 2054, 2057, 2060, 2063, 2066, 2069, 2072, 2075, 2078),]
  

#replace depth of 2 by 1 :
SOMLIT_oxy <- SOMLIT_oxy %>% 
  mutate(PROF_NUM = case_when(PROF_NUM == 2 ~ 1, TRUE ~ PROF_NUM))
 
#save :
#write.table(SOMLIT_oxy, "DATA_SOMLIT_02-22_oxy.csv", sep=";", col.names = TRUE, row.names = FALSE) 
...
#### *Measure SOMLIT oxygen (2002-2022) every tuesday*  
  
→ Fill in the gaps: adding lines  
  


```{r SOMLIT oxygen (2002-2022) - adding lines manually, echo=FALSE, warning=FALSE, message=FALSE}

#mesures non-regulieres, des fois mardi, des fois jeudi
#number of measure by month by year :
Nb_by_month_oxy <- summarise(group_by(SOMLIT_oxy, year.month), NB_OBS_M=n()) 
#must be 8 or 10

SOMLIT_oxy$datetime <- as.character(SOMLIT_oxy$datetime)

#adding lines manually :
empty_mat = matrix(ncol = 9, nrow = 68)
empty_rows <- data.frame(empty_mat) %>% 
  rename(datetime=X1, PROF_NUM=X2, T=X3, S=X4, O=X5, oxy_umol_kg=X6, year=X7, month=X8, year.month=X9)

SOMLIT_oxy <- rbind(empty_rows, SOMLIT_oxy)
##

#adding missing dates manually :
SOMLIT_oxy[c(1,2),"datetime"] <- "2002-02-27 09:00:00"
SOMLIT_oxy[c(1,2),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(3,4),"datetime"] <- "2004-10-27 09:00:00"
SOMLIT_oxy[c(3,4),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(5,6),"datetime"] <- "2002-12-26 09:00:00"
SOMLIT_oxy[c(5,6),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(7,8),"datetime"] <- "2007-04-10 09:00:00"
SOMLIT_oxy[c(7,8),"PROF_NUM"] <- c("1", "50")

SOMLIT_oxy[c(9,10),"datetime"] <- "2007-12-25 09:00:00"
SOMLIT_oxy[c(9,10),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(11,12),"datetime"] <- "2008-08-12 09:00:00"
SOMLIT_oxy[c(11,12),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(13,14),"datetime"] <- "2011-12-27 09:00:00"
SOMLIT_oxy[c(13,14),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(15,16),"datetime"] <- "2012-08-14 09:00:00"
SOMLIT_oxy[c(15,16),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(17,18),"datetime"] <- "2013-08-13 09:00:00"
SOMLIT_oxy[c(17,18),"PROF_NUM"] <- c("1", "50")

SOMLIT_oxy[c(19,20),"datetime"] <- "2014-08-12 09:00:00"
SOMLIT_oxy[c(19,20),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(21,22),"datetime"] <- "2015-08-18 09:00:00"
SOMLIT_oxy[c(21,22),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(23,24),"datetime"] <- "2016-12-27 09:00:00"
SOMLIT_oxy[c(23,24),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(25,26),"datetime"] <- "2017-02-28 09:00:00"
SOMLIT_oxy[c(25,26),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(27,28),"datetime"] <- "2017-12-26 09:00:00"
SOMLIT_oxy[c(27,28),"PROF_NUM"] <- c("1", "50")

SOMLIT_oxy[c(29,30),"datetime"] <- "2018-04-10 09:00:00"
SOMLIT_oxy[c(29,30),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(31,32),"datetime"] <- "2018-08-14 09:00:00"
SOMLIT_oxy[c(31,32),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(33,34),"datetime"] <- "2018-12-25 09:00:00"
SOMLIT_oxy[c(33,34),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(35,36),"datetime"] <- "2019-08-13 09:00:00"
SOMLIT_oxy[c(35,36),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(37,38),"datetime"] <- "2019-12-24 09:00:00"
SOMLIT_oxy[c(37,38),"PROF_NUM"] <- c("1", "50")

SOMLIT_oxy[c(39,40),"datetime"] <- "2019-12-31 09:00:00"
SOMLIT_oxy[c(39,40),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(41,42),"datetime"] <- "2020-03-17 09:00:00"
SOMLIT_oxy[c(41,42),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(43,44),"datetime"] <- "2020-03-24 09:00:00"
SOMLIT_oxy[c(43,44),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(45,46),"datetime"] <- "2020-03-31 09:00:00"
SOMLIT_oxy[c(45,46),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(47,48),"datetime"] <- "2020-04-07 09:00:00"
SOMLIT_oxy[c(47,48),"PROF_NUM"] <- c("1", "50")

SOMLIT_oxy[c(49,50),"datetime"] <- "2020-04-14 09:00:00"
SOMLIT_oxy[c(49,50),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(51,52),"datetime"] <- "2020-04-28 09:00:00"
SOMLIT_oxy[c(51,52),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(53,54),"datetime"] <- "2011-12-27 09:00:00"
SOMLIT_oxy[c(53,54),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(55,56),"datetime"] <- "2021-11-16 09:00:00"
SOMLIT_oxy[c(55,56),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(57,58),"datetime"] <- "2021-11-30 09:00:00"
SOMLIT_oxy[c(57,58),"PROF_NUM"] <- c("1", "50")

SOMLIT_oxy[c(59,60),"datetime"] <- "2021-12-28 09:00:00"
SOMLIT_oxy[c(59,60),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(61,62),"datetime"] <- "2022-01-04 09:00:00"
SOMLIT_oxy[c(61,62),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(63,64),"datetime"] <- "2022-10-18 09:00:00"
SOMLIT_oxy[c(63,64),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(65,66),"datetime"] <- "2022-12-27 09:00:00"
SOMLIT_oxy[c(65,66),"PROF_NUM"] <- c("1", "50")
SOMLIT_oxy[c(67,68),"datetime"] <- "2006-12-26 09:00:00"
SOMLIT_oxy[c(67,68),"PROF_NUM"] <- c("1", "50")
##

SOMLIT_oxy <- SOMLIT_oxy %>% arrange(datetime)


#ajout row 2020-08-11 for 50m
SOMLIT_oxy <- insertRows(SOMLIT_oxy, 1924, new = NA)

SOMLIT_oxy <- SOMLIT_oxy %>% 
  mutate(datetime = str_replace_na(datetime, replacement = "NA"),
         PROF_NUM = str_replace_na(PROF_NUM, replacement = "NA")) %>% 
  mutate(datetime = str_replace(datetime, "NA", "2020-08-11 09:00:00"),
         PROF_NUM = str_replace(PROF_NUM, "NA", "50"))

#ajout row 2020-08-19 for 50m
SOMLIT_oxy <- insertRows(SOMLIT_oxy, 1926, new = NA)

SOMLIT_oxy <- SOMLIT_oxy %>% 
  mutate(datetime = str_replace_na(datetime, replacement = "NA"),
         PROF_NUM = str_replace_na(PROF_NUM, replacement = "NA")) %>% 
  mutate(datetime = str_replace(datetime, "NA", "2020-08-19 09:00:00"),
         PROF_NUM = str_replace(PROF_NUM, "NA", "50"))
##

#MAJ year, month and year.month :
SOMLIT_oxy <- SOMLIT_oxy %>% 
...
SOMLIT_oxy <- SOMLIT_oxy[-c(105,106),]

#save :
#write.table(SOMLIT_oxy, "DATA_SOMLIT_02-22_oxy_CLEAN.csv", sep=";", col.names = TRUE, row.names = FALSE)
...
```{r interpolation SOMLIT_oxy_CLEAN - surface & 50m, echo=FALSE, warning=FALSE, message=FALSE}

#surface :
SOMLIT_oxy_surf <- SOMLIT_oxy %>% 
  dplyr::filter(PROF_NUM == 1)

#50m :
SOMLIT_oxy_prof <- SOMLIT_oxy %>% 
  dplyr::filter(PROF_NUM == 50)
##

#interpolation surface :
T_interpol_surf <- na_seadec(SOMLIT_oxy_surf$T, algorithm = "interpolation", find_frequency = T)
S_interpol_surf <- na_seadec(SOMLIT_oxy_surf$S, algorithm = "interpolation", find_frequency = T)
O_interpol_surf <- na_seadec(SOMLIT_oxy_surf$O, algorithm = "interpolation", find_frequency = T)

#interpolation 50m :
T_interpol_prof <- na_seadec(SOMLIT_oxy_prof$T, algorithm = "interpolation", find_frequency = T)
S_interpol_prof <- na_seadec(SOMLIT_oxy_prof$S, algorithm = "interpolation", find_frequency = T)
O_interpol_prof <- na_seadec(SOMLIT_oxy_prof$O, algorithm = "interpolation", find_frequency = T)
##
...
#surface :
SW_density_somlit_interpol_surf <- rho(S = S_interpol_surf, T=T_interpol_surf, P=0)/1000
convert_o2_somlit_interpol_surf <- 44.66/SW_density_somlit_interpol_surf


#50m
SW_density_somlit_interpol_prof <- rho(S = S_interpol_prof, T=T_interpol_prof, P=0)/1000
convert_o2_somlit_interpol_prof <- 44.66/SW_density_somlit_interpol_prof
##


#surface :
SOMLIT_oxy_interpol_surf <- SOMLIT_oxy_surf %>% 
  mutate(T = T_interpol_surf,
         S = S_interpol_surf,
         O = O_interpol_surf,
         oxy_umol_kg = O*convert_o2_somlit_interpol_surf)

#50m :
SOMLIT_oxy_interpol_prof <- SOMLIT_oxy_prof %>% 
  mutate(T = T_interpol_prof,
         S = S_interpol_prof,
         O = O_interpol_prof,
         oxy_umol_kg = O*convert_o2_somlit_interpol_prof)
##


#save :
#write.table(SOMLIT_oxy_interpol_surf, "DATA_SOMLIT_02-22_oxy_CLEAN_impute_0m.csv", sep=";", col.names = TRUE, row.names = FALSE)
#write.table(SOMLIT_oxy_interpol_prof, "DATA_SOMLIT_02-22_oxy_CLEAN_impute_50m.csv", sep=";", col.names = TRUE, row.names = FALSE)
```
...
## Importation data EOL (seanoe) : sept.2013 - october.2022   
...
test <- data_EOL_raw[c(31968:40297),]
...
test2 <- data_EOL_raw[c(23730:31967),]
...
EOL_22 <- test %>% 
  ...
  dplyr::select(datetime, temp_eol_c, sal_eol_psu, oxy_umol_kg)
...
EOL_01_11_22 <- read_delim("../Data/EOL_01_11_22.csv", 
  ...
  dplyr::select(datetime, salinity) #je sais pas ce que c'est ?
##
...
- 1 data/jour à 09h (EOL 1)  
- 2 data/jour : 09h et 16h (EOL 2)  
- hourly data (EOL 3)  
...
GG <- read_delim("../Data/EOL_raw_complete_1323.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
...
#length(unique(EOL_test1$datetime)) #2628 different dates


EOL_test1 <- EOL_test1 %>% mutate(cond = case_when(NB_OBS_DAY > 1 ~ "TRUE", TRUE ~ "FALSE"))

#tab of days with more than 1 observation : mean of observations
tab1 <- EOL_test1 %>% filter(cond=="TRUE") %>% 
  group_by(datetime) %>% 
  mutate(temp_eol_c = mean(temp_eol_c, na.rm = TRUE),
         sal_eol_psu = mean(sal_eol_psu, na.rm = TRUE),
         oxy_umol_kg = mean(oxy_umol_kg, na.rm = TRUE)) %>% 
  dplyr::select(-Hour)

tab1 <- distinct(tab1) %>% 
  mutate(Hour = "09:00:00")

#tab of days of 1 observation :
tab2 <- EOL_test1 %>% filter(cond=="FALSE")


#tableau clean : 2628 obs
#period : sept.2013 - mar.2023
...
EOL_1 <- EOL_1 %>% arrange(datetime)
##

#bcp de data manquantes
#aout + septembre 2017
#été 2016....
 

#save :
#write.table(EOL_1, "EOL_13-23_1perday_morning.csv", sep=";", col.names = TRUE, row.names = FALSE)


```


### *Measure EOL : twice per day (at 8AM and 4PM)*  faire plus loin
  

```{r Measure EOL : 2 per day (morning & afternoon), echo=FALSE, warning=FALSE, message=FALSE}

EOL_test2 <- data_EOL_complete %>%
  filter(Hour == "08:00:00" | Hour == "09:00:00" | Hour == "10:00:00" | Hour == "15:00:00" | Hour == "16:00:00" | Hour == "17:00:00") %>% 
  group_by(datetime) %>% 
  mutate(NB_OBS_DAY = n(),
         datetime = as.POSIXct(datetime, format="%Y-%m-%d"))
##

EOL_test2 <- EOL_test2 %>% mutate(cond = case_when(NB_OBS_DAY > 2 ~ "TRUE", TRUE ~ "FALSE"))


#tab of days of more than 2 observations :
tab1_2 <- EOL_test2 %>% filter(cond=="TRUE") 


tab1_2_morning <- tab1_2 %>% dplyr::filter(Hour == "08:00:00" | Hour == "09:00:00" | Hour == "10:00:00") %>% 
  group_by(datetime) %>% 
  mutate(temp_eol_c = mean(temp_eol_c, na.rm = TRUE),
         sal_eol_psu = mean(sal_eol_psu, na.rm = TRUE),
         oxy_umol_kg = mean(oxy_umol_kg, na.rm = TRUE)) %>% 
  dplyr::select(-c(Hour, NB_OBS_DAY))

tab1_2_morning <- distinct(tab1_2_morning) %>% 
  mutate(Hour = "09:00:00")


tab1_2_afternoon <- tab1_2 %>% dplyr::filter(Hour == "15:00:00" | Hour == "16:00:00" | Hour == "17:00:00") %>% 
  group_by(datetime) %>% 
  mutate(temp_eol_c = mean(temp_eol_c, na.rm = TRUE),
         sal_eol_psu = mean(sal_eol_psu, na.rm = TRUE),
         oxy_umol_kg = mean(oxy_umol_kg, na.rm = TRUE)) %>% 
...
tab1_2_afternoon <- distinct(tab1_2_afternoon) %>% 
  mutate(Hour = "16:00:00")

#merge :
tab1_2 <- full_join(tab1_2_morning, tab1_2_afternoon)

#tab of days of less than 2 observations :
tab2_2 <- EOL_test2 %>% filter(cond=="FALSE") 

tab2_2_morning <- tab2_2 %>% dplyr::filter(Hour == "08:00:00" | Hour == "09:00:00" | Hour == "10:00:00") %>% 
  group_by(datetime) %>% 
  mutate(temp_eol_c = mean(temp_eol_c, na.rm = TRUE),
         sal_eol_psu = mean(sal_eol_psu, na.rm = TRUE),
         oxy_umol_kg = mean(oxy_umol_kg, na.rm = TRUE)) %>% 
...
tab2_2_morning <- distinct(tab2_2_morning) %>% 
  mutate(Hour = "09:00:00")

tab2_2_afternoon <- tab2_2 %>% dplyr::filter(Hour == "15:00:00" | Hour == "16:00:00" | Hour == "17:00:00") %>% 
  group_by(datetime) %>% 
  mutate(temp_eol_c = mean(temp_eol_c, na.rm = TRUE),
         sal_eol_psu = mean(sal_eol_psu, na.rm = TRUE),
         oxy_umol_kg = mean(oxy_umol_kg, na.rm = TRUE)) %>% 
...
tab2_2_afternoon <- distinct(tab2_2_afternoon) %>% 
  mutate(Hour = "09:00:00")

#merge :
tab2_2 <- full_join(tab2_2_morning, tab2_2_afternoon)

##


#tableau clean : 4756 obs
#period : sept.2013 - mar.2023

EOL_2 <- full_join(tab1_2, tab2_2) %>% 
  group_by(datetime) %>% 
  mutate(NB_OBS_DAY = n()) %>% 
  select(-cond)

EOL_2 <- EOL_2 %>% arrange(datetime)
...
#save :
#write.table(EOL_2, "EOL_13-23_2perday_morning_afternoon.csv", sep=";", col.names = TRUE, row.names = FALSE)
...
#on prend a partir de 2018 pour avoir 4 ans complets
...
#tableau clean
EOL_3 <- EOL_test3 %>%
  dplyr::select(datetime, Hour, temp_eol_c, sal_eol_psu, oxy_umol_kg, Year, Month, Year.Month, Year.Month.Day, dMonth, NB_OBS_DAY)


#save :
#write.table(EOL_3, "EOL_18-22_hourly.csv", sep=";", col.names = TRUE, row.names = FALSE)
