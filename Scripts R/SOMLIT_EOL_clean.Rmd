...
## Importation data EOL : 2013-2022   
...
data_EOL <- read_csv("../Data/SEANOE_EOL.csv", 
...
EOL_22 <- EOL_22 %>% 
  mutate(month = case_when(month == "Nov" ~ "11",
                           ...
                           month == "Feb" ~ "2",
                           month == "Mar" ~ "3", TRUE ~ ""))
EOL_22$month <- as.double(EOL_22$month)

EOL_22 <- EOL_22 %>% 
  unite(col='datetime', c('year', 'month', 'day'), sep='-0') %>% 
  unite(col='datetime', c('datetime', 'hour'), sep=' ') %>% 
  mutate(datetime = str_replace(datetime, "011", "11")) %>%
  rename(temp_eol_c = temperature, sal_eol_psu = salinity, oxy_eol_mll = oxygen_umol_kg) %>% 
  select(datetime, temp_eol_c, sal_eol_psu, oxy_eol_mll)
 

#data 2022 SEAFET Samir (only temperature):
#from nov. 2022 to march 2023 :
...
data_EOL <- rbind(data_EOL, EOL_22)
data_EOL <- rbind(data_EOL, EOL_seafet_22)
data_EOL <- data_EOL %>% arrange(datetime)

#save :
write.table(data_EOL, "EOL_raw_complete_1323.csv", sep=";", col.names = TRUE, row.names = FALSE)
...
Nb_by_year <- summarise(group_by(data_EOL, Year), NB_OBS_Y=n()) 
...
Nb_by_year %>% 
  ...
  ggtitle("Evolution of number of observations per year - EOL (2007-2022)")
...
Nb_by_month <- summarise(group_by(data_EOL, Year.Month), NB_OBS_M=n()) 
...
Nb_by_month %>% 
  ...
  scale_x_date(name="") + 
  scale_y_continuous(name="") +
  ggtitle("Evolution of number of observations per month- EOL (2007-2022)")
```
...
#number of measure/month :
Nb_by_day <- summarise(group_by(data_EOL, Year.Month.Day), NB_OBS_M=n()) 
...
Nb_by_day %>% 
  ...
  scale_x_date(name="") + 
  scale_y_continuous(name="") +
  ggtitle("Evolution of number of observations per day- EOL (2007-2022)")
...
EOL_1 <- full_join(tab1, tab2) %>% 
  group_by(datetime) %>% 
  mutate(NB_OBS_DAY = n()) %>% 
  select(datetime, Hour, temp_eol_c, sal_eol_psu, oxy_eol_mll, Year, Month, Year.Month, Year.Month.Day, dMonth, NB_OBS_DAY)
