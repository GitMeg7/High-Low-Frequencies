...
graph_wind_eol %>%  
  ...
  aes(x=year, y=mean) +
  geom_point(shape=1, col="black", size=3) +
  geom_line() +
...
SOMLIT_pcO2 <- read_delim("../Data/DATA_SOMLIT_07-22_impute_0m.csv", 
...
## Annual cycle of seawater pCO~2~ at Point B (2007-2022) :  
...
## Calculation of NpCO~2~ & TpCO~2~ and deltas :  
...
```{r NpCO2 & TpCO2 & deltas calculations, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10,fig.height=6}

#formula NpCO2 : pCO2w obs * e(0.0423*Tmean-Tobs)
#formula TpCO2 : pCO2mean * exp[0.0423(Tobs - Tmean)]

pco2_bio <- read_delim("../Data/DATA_SOMLIT_07-22_impute_0m.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
...
## Atmospheric and seawater pCO~2~ (µatm) according to time (2007-2022) :  
...
atmos_water_pCO2_plot_long <- pco2_bio %>% 
  rename(`pCO2 seawater` = impute_pCO2_w, `pCO2 atmospheric` = impute_pCO2_atmos, NpCO2 = NpCO2) %>% 
  pivot_longer(cols = c(`pCO2 seawater`, `pCO2 atmospheric`, "NpCO2"), names_to = "Legend", values_to = "value")
...
tab_flux <- read_delim("../Data/DATA_SOMLIT_07-22_impute_0m_deltas_pCO2.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
...
tab_flux %>% 
  ...
  scale_y_continuous(name="Fluxes (mol of C/m²/y)", labels = label_scientific(), n.breaks=8) +
...
tab_flux %>% 
  ...
  scale_x_date(name="", date_breaks = "2 year", date_minor_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(name="Fluxes (mol C/m²/h)") +
...
### Summary table of 2 calculation methods results (Wanninkhof 1992 & 2014) :  
  

```{r 2 methods comparison table, echo=FALSE, warning=FALSE, message=FALSE}

summary_table <- data.frame(tab_flux$datetime, tab_flux$Flux_wan_92, tab_flux$Flux_CO2_2014)

colnames(summary_table) <- c("Datetime", "Flux_wan_92", "Flux_wan_2014")

#summary_table <- summary_table %>% dplyr::filter(Datetime <= "2021-12-28")
...
fluxes_co2_plot <- tab_flux_long %>% 
  ...
  ggtitle("") +
  geom_line(aes(x=datetime, y=value, color= Legend), size=0.5) +
  scale_y_continuous(name="Fluxes (mol of C/m²/h)", labels = label_scientific(), n.breaks=8) + 
  scale_x_date(name="", date_breaks = "2 year", date_minor_breaks = "1 year", date_labels = "%Y") +
...
annual_fluxes %>% 
  ...
  scale_y_continuous(name="Fluxes (mol of C/m²/h)", labels = label_scientific(), n.breaks = 10) +
  ggtitle("") 
...
## Comparison CO~2~ fluxes at Point B vs offshore (Carioca)  
  
  

```{r comparison Pt B - Carioca, echo=FALSE, warning=FALSE, message=FALSE}

Carioca <- read_delim("../Data/pCO2_carioca/pco2_AT_CT_98_19_Cathy2022.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE) %>% dplyr::filter(PRESS == 5) %>% 
