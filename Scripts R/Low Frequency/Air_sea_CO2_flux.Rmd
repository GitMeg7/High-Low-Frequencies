...
fluxes_co2_plot <- tab_flux_long %>% 
  ...
  ggtitle("") +
  geom_line(aes(x=datetime, y=value, color= Legend), size=0.5) +
  scale_y_continuous(name=expression(paste("Flux (mol de C ", m^-2, " ", h^-1, ")")), labels = label_scientific(), n.breaks=8) + 
  scale_x_date(name="", date_breaks = "2 year", date_minor_breaks = "1 year", date_labels = "%Y") +
...
annual_fluxes %>% 
  ...
  scale_y_continuous(name=expression(paste("Flux (mol de C ", m^-2, " ", h^-1, ")")), labels = label_scientific(), n.breaks = 10) +
