...
## pCO2 bio : Point B (2007-Dec.2021)
...
data_pco2 <- read_delim("../../Data/DATA_SOMLIT_07-22_impute_0m_deltas_pCO2.csv", 
                        delim = ";", escape_double = FALSE, trim_ws = TRUE) %>% 
  dplyr::filter(datetime <= "2021-12-28")

## Amplitude :
data_pco2 <- data_pco2 %>% 
  group_by(Year) %>% 
  mutate(ampli_bio = max(delta_pCO2_bio) - min(delta_pCO2_bio))
...
FF2 <- kendallSeasonalTrendTest(delta_pCO2_bio ~ month(datetime) + year(datetime), data = data_pco2)
FF2$estimate
#ts(variable)
#smk.test(ts, continuity=TRUE)
```

```{r pCO2 bio - bates plots, echo=FALSE, warning=FALSE, message=FALSE, fig.width=12,fig.height=4}
...
plot_ano_pCO2bio <- ggplot(data = anomalies, aes(x = datetime, y = bio_pco2_ano), na.rm=TRUE) +
  ...
  labs(title="",x="", y="ΔpCO2 (bio)  (µatm)") +
  annotate(geom="text", x=as.Date("2010-03-02"), y=80, label="slope : 3.58 ± 0.15 *", color="black")

plot_delta_bio <- data_pco2 %>% 
  ggplot() +
  aes(x=datetime, y=delta_pCO2_bio) + 
  geom_line(size=0.8, col = "#096A09", linewidth = 0.5) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_x_date(name="", date_breaks="2 year", date_minor_breaks="1 years", date_labels = "%Y") +
  scale_y_continuous(name="ΔpCO2 (bio)  (µatm)")
#  ggtitle("Surface delta pCO2 bio variations (2007-sept.2022)")



plot_grid_delta_bio <- plot_grid(plot_delta_bio, plot_ano_pCO2bio, align='vh', ncol=2)

plot_grid_delta_bio

 # ggsave(filename = "delta_bio_07-21.jpg", # Nommez le fichier dans lequel vous voulez enregistrer, ajoutez l'extension du format de fichier que vous voulez utiliser (ex. pdf).
 #         plot = plot_grid_delta_bio, # Fournir le nom de l'objet plot dans R
 #         height = 4, # Fournir les dimensions voulues
 #         width = 12, 
 #         units = "in") 
...
plot_ts_pco2_bio_less_season <- autoplot(ts_pco2_bio_less_season) + 
  ...
  ggtitle("Surface delta pCO2 bio variations (without seasonality)") +
  geom_smooth(method=lm) +
  annotate(geom="text", x=2010, y=100, label="slope : 2.50 ± 0.26 *", color="black")

plot_ts_pco2_bio_less_season

FFpco2bio <- kendallSeasonalTrendTest(delta_pCO2_bio ~ month(datetime) + year(datetime), data = data_pco2)
FFpco2bio$estimate
...
FF3 <- kendallSeasonalTrendTest(Flux_wan_92 ~ month(datetime) + year(datetime), data = data_fluxes)
FF3$estimate
...
data_somlit_prof <- read_delim("../../Data/DATA_SOMLIT_07-22_impute_50m.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
...
```{r seasonal_slopes, echo=FALSE, warning=FALSE, message=FALSE, fig.width=5,fig.height=6}

seasonal_slopes_pco2w_surf <- as.data.frame(FFpCO2_w$seasonal.estimates) %>% 
  dplyr::select(slope) %>% 
  rename(pco2_slope = slope)

seasonal_slopes_temp_surf <- as.data.frame(FFtemp$seasonal.estimates) %>% 
  dplyr::select(slope) %>% 
  rename(temp_slope = slope)

seasonal_slopes <- data.frame(ID = c(1:12), Months = c("Jan", "Fev", "Mar", "Avr", "Mai", "Juin", 
                                         "Juil", "Août", "Sept", "Oct", "Nov", "Dec"), 
                              pco2_slope = seasonal_slopes_pco2w_surf$pco2_slope,
                              temp_slope = seasonal_slopes_temp_surf$temp_slope)

seasonal_slopes[,c(2:4)]
...
plot1 <- seasonal_slopes %>% 
  ...
  scale_x_continuous(name="", n.breaks = 12, labels =NULL)
