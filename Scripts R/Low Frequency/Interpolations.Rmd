...
```{r interpolation temp & salinity, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6,fig.height=4}
...
SOMLIT_pcO2 %>% 
  ...
  scale_x_discrete(name="", limits=c("2007":"2022"))

SOMLIT_pcO2 <- SOMLIT_pcO2 %>% 
  mutate(temperature = as.numeric(temperature))


# ggplot_na_gapsize(SOMLIT_pcO2$temperature)


ggplot_na_imputations(SOMLIT_pcO2$temperature, SOMLIT_pcO2$impute_temp, x_axis_labels = SOMLIT_pcO2$datetime, xlab = "", ylab="Temp. (°C)", 
                      title="Point B - Temperature time series (°C) - period 2007-2022", color_lines = "lightslategray",
                      color_points = "steelblue", size_points = 1.2, size_imputations = 1.7) +
  scale_x_date(date_breaks = "2 years", date_labels = "%Y") 
  
ggplot_na_imputations(SOMLIT_pcO2$salinity, SOMLIT_pcO2$impute_sal, x_axis_labels = SOMLIT_pcO2$datetime, xlab = "", ylab="Salinity", title="Point B - Salinity time series - period 2007-2022")
...
### Data of atmospheric pCO~2~ (µatm):  
  
period : Jan.2007 - Dec.2021 
...
```{r importation pCO2 air, echo=FALSE, warning=FALSE, message=FALSE, fig.width=7,fig.height=4}
 ...
 pCO2_air <- read_delim("../../Data/atmospheric_pco2_icos/DATA_pCO2_atmos_06-22.csv", 
     delim = ";", escape_double = FALSE, trim_ws = TRUE) %>% 
   dplyr::filter(datetime >= "2007-01-01" & datetime <= "2021-12-31") %>% 
...
# merge1 <- merge1 %>% 
#   mutate(pCO2_CORSE = str_replace_na(pCO2_CORSE, replacement = "NA"),
#          pCO2_PR = str_replace_na(pCO2_PR, replacement = "NA"),
#          pCO2_LAMP = str_replace_na(pCO2_LAMP, replacement = "NA"),
