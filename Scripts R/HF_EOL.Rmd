...
# a voir aussi en triant la data par espace temps regulier :
# exemple : 
# 1 : par jour
# 2 : 2 fois par jour
# 3 : par heure ?
...
## **Nombre de mesures EOL**  
  
- per year :  
  
  

```{r Nombre de mesures EOL par an, echo=FALSE, warning=FALSE, message=FALSE}

#number of measure by year :
Nb_by_year <- summarise(group_by(EOL_RAW, Year), NB_OBS_Y=n()) 


#plot number of measure by year (2013-2022)
Nb_by_year %>% 
  ggplot() +
  ...
  ggtitle("Evolution of number of observations by year at EOL buoy (2007-2022)")
...
```{r Nombre de mesures EOL par mois, echo=FALSE, warning=FALSE, message=FALSE}

#number of measure by month :
Nb_by_month <- summarise(group_by(EOL_RAW, Year.Month), NB_OBS_M=n()) 
...
```{r visualisation data temperature EOL, echo=FALSE, message=FALSE, warning=FALSE}

EOL_RAW %>% 
  ggplot() +
  aes(x=datetime, y=temperature) + 
  geom_point(size=0.6, col="#22427C") + 
  scale_x_datetime(name="") + 
  scale_y_continuous(name="Temp. (°C)") + 
  ggtitle("High frequency surface data of temperature (2013-2022)")


```
```{r importation data SOMLIT, echo=FALSE, message=FALSE, warning=FALSE}

#periode janvier 2007 - septembre 2022 :
SOMLIT <- read_delim("../Data/Somlit_Extraction_ctd_dernier.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)

#selection profondeur 1m (a refaire avec moyenne de 1 a 3m)
SOMLIT <- SOMLIT %>% 
  dplyr::filter(PROFONDEUR == 1) %>% 
  select(DATE, TEMPERATURE, SALINITE) %>% 
  rename(datetime = DATE, temp_Pt_B = TEMPERATURE, sal_Pt_B = SALINITE) %>% 
...
#fusion
fusion <- full_join(EOL, SOMLIT)
fusion <- fusion %>% arrange(datetime) %>% 
  dplyr::filter(datetime > "2013-09-01" & datetime < "2022-09-14") %>% 
  mutate(temp_Pt_B = case_when(temp_Pt_B >= 999 ~ NA_real_ ,TRUE ~ temp_Pt_B))
...
fusion %>% 
  ggplot() +
  geom_point(aes(x=datetime, y=temperature), size=0.7, col="red") + 
  geom_point(aes(x=datetime, y=temp_Pt_B), size=1, col="#22427C") + 
  scale_x_datetime(name="") + 
  scale_y_continuous(name="Temp. (°C)") + 
  ggtitle("HF (EOL) vs LF (Point B) surface data of temperature (2013-2022)")
...
fusion_scatterplot <- left_join(SOMLIT, EOL, by="datetime")
fusion_scatterplot <- fusion_scatterplot %>% 
  dplyr::filter(datetime > "2013-09-01" & datetime < "2022-09-14") %>% 
  mutate(temp_Pt_B = case_when(temp_Pt_B >= 999 ~ NA_real_ ,TRUE ~ temp_Pt_B))

#scatterplot EOL/SOMLIT
fusion_scatterplot %>% 
  ggplot() + 
  ggtitle("Scatter plot EOL vs Point B") +
  aes(x=temp_Pt_B, y=temperature) + 
  geom_point(size=0.8) + 
  geom_smooth(method=lm) + 
  scale_x_continuous(name="Temperature Point B") + 
  scale_y_continuous(name="Temperature EOL")

summary(lm(fusion_scatterplot$temperature ~ fusion_scatterplot$temp_Pt_B))
...
plot_ano_sal
```
## Part 2 : Trend analysis with Fourrier power spectral density
...
## Part 3 : Trend analysis with autocorrelation
...
## Part 4 : Trend analysis with raw dataset
