...
#### Part 1 : Trend analysis by residuals : raw dataset (jan. 2014 - dec.2022)  
*By autocorrelation + decompose function : not possible because of gaps*  
  
  
###### [Table and plot of monthly means (2014-2022) - EOL temperature & salinity]{style="color:blue"}  
...
###### [Table of time series anomalies regression analyses (2014-2022) - EOL temperature & salinity]{style="color:green"}  
...
**Plot of anomalies : Temperature (°C) (2014-2022) - EOL**  
...
plot_ano_temp_EOL_raw <- ggplot(data = anomalies, aes(x = datetime, y = Temperature_ano), na.rm=TRUE) +
  ...
  labs(title="EOL - Temperature anomalies + trend (2014-2022)",x="", y="Temp. (°C)") +
  annotate(geom="text", x=as.POSIXct("2015-12-03 01:00:00"), y=5, label="slope : 0.086 ± 0.003 *", color="black")
...
plot_temp_EOL_raw <- EOL_RAW %>% 
  ...
  scale_x_datetime(name="") +
  scale_y_continuous(name="Temp. (°C)") +
  ggtitle("EOL temperature time series (°C) 2014-2022")
...
**Plot of anomalies : Salinity (2014-2022) - EOL**  
...
plot_ano_sal_EOL_raw <- ggplot(data = anomalies, aes(x = datetime, y = Salinity_ano), na.rm=TRUE) +
  ...
  geom_point(colour="blue", na.rm=TRUE, size=0.65) + 
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="EOL - Salinity anomalies + trend (2014-2022)",x="", y="Temp. (°C)") +
  annotate(geom="text", x=as.POSIXct("2015-12-03 01:00:00"), y=1, label="slope : 0.0037 ± 0.0004 *", color="black")
...
plot_sal_EOL_raw <- EOL_RAW %>% 
  ...
  ggtitle("EOL salinity time series (2014-2022)")
...
#### Part 2 : Trend analysis by residuals : daily dataset (morning 8AM-10AM) jan.2014 - dec.2022  
...
###### [EOL 1 : Table and plot of monthly means (2014-2022) - EOL temperature & salinity]{style="color:blue"}  
  
To do monthly means, imputations of missing data of EOL by data of SOMLIT :  
  
  
```{r monthly imputations somlit to eol, echo=FALSE, warning=FALSE, message=FALSE}

##importation SOMLIT dataset : 
data_somlit <- read_delim("../Data/DATA_SOMLIT_07-22_impute_0m.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)


##selection of data :

#year 2014 : february, march and december :
year_2014 <- data_somlit[c(372, 373, 374, 375, 376, 377, 378, 414, 415, 416, 417, 418),]
year_2014 <- year_2014 %>% 
  dplyr::select(c(datetime, impute_temp, impute_sal))

#year 2016 : may, august and september :
year_2016 <- data_somlit[c(488, 489, 490, 491, 492, 502, 503, 504, 505, 506, 507, 508, 509, 510),]
year_2016 <- year_2016 %>% 
  dplyr::select(c(datetime, impute_temp, impute_sal))

#year 2017 : august and september :
year_2017 <- data_somlit[c(555, 556, 557, 558, 559, 560, 561, 562, 563),]
year_2017 <- year_2017 %>% 
  dplyr::select(c(datetime, impute_temp, impute_sal))

#year 2019 : april :
year_2019 <- data_somlit[c(642, 643, 644, 645, 646),]
year_2019 <- year_2019 %>% 
  dplyr::select(c(datetime, impute_temp, impute_sal))


##merge of missing years somlit :
missing_values_somlit <- full_join(year_2014, year_2016, by=c("datetime", "impute_temp", "impute_sal"))
missing_values_somlit <- full_join(missing_values_somlit, year_2017, by=c("datetime", "impute_temp", "impute_sal"))
missing_values_somlit <- full_join(missing_values_somlit, year_2019, by=c("datetime", "impute_temp", "impute_sal"))


##merge with EOL :
EOL_daily <- full_join(EOL_daily, missing_values_somlit, by=c("datetime", "temperature"="impute_temp", "salinity"="impute_sal"))

EOL_daily <- EOL_daily %>% arrange(datetime)
...
#save :
#write.table(EOL_daily, "EOL_13-23_1perday_morning_FILL.csv", sep=";", col.names = TRUE, row.names = FALSE)
...
###### [EOL 1 : Table of time series anomalies regression analyses (2014-2022) - EOL temperature & salinity]{style="color:green"}  
...
**EOL 1 : Plot of anomalies : Temperature (°C) (2014-2022) - EOL**  
...
plot_temp_EOL_1 <- EOL_daily %>% 
  ...
  scale_x_datetime(name="") +
  scale_y_continuous(name="Temp. (°C)") +
  ggtitle("EOL time series observations of temperature (°C) 2014-2022")
...
**EOL 1 : Plot of anomalies : Salinity (2014-2022) - EOL**  
...
#### Part 3 : Trend analysis by residuals : daily dataset x2 (morning + afternoon) jan.2014 - dec.2022  
  
  
→ daily variations : mornings/afternoons  
  

```{r importation Data EOL 2 - daily, echo=FALSE, warning=FALSE, message=FALSE}

EOL_daily_2 <- read_delim("../Data/EOL_13-23_2perday_morning_afternoon.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE) %>% 
  rename(temperature=temp_eol_c, salinity=sal_eol_psu) %>% 
  mutate(datetime = as.POSIXct(datetime)) %>% 
  dplyr::filter(datetime >= "2014-01-07 01:00:00" & datetime <= "2022-12-31 01:00:00")
...
EOL_daily_2 <- EOL_daily_2 %>% 
  mutate(Hour = as.character(Hour))

tab_morning <- EOL_daily_2 %>% 
  dplyr::filter(Hour == "09:00:00") %>% 
  rename(morning = temperature)

tab_afternoon <- EOL_daily_2 %>% 
  dplyr::filter(Hour == "16:00:00") %>% 
  rename(afternoon = temperature)

test <- full_join(tab_morning, tab_afternoon)
test <- test %>% arrange(datetime)

#plot :
test %>% 
  ggplot() + 
  geom_point(aes(x=datetime, y=morning), col="blue") + 
  geom_point(aes(x=datetime, y=afternoon), col="pink")
...
###### [EOL 2 : Table and plot of monthly means (2014-2022) - EOL temperature & salinity]{style="color:blue"}  
  
  
Same missing values : imputations of missing data by data of SOMLIT  
  

```{r EOL 2 - calcul monthly means - temperature & salinity, echo=TRUE, warning=FALSE, message=FALSE}

##merge with EOL_2 :
EOL_daily_2 <- full_join(EOL_daily_2, missing_values_somlit, by=c("datetime", "temperature"="impute_temp", "salinity"="impute_sal"))

EOL_daily_2 <- EOL_daily_2 %>% arrange(datetime)
...
EOL_daily_2 <- EOL_daily_2 %>% 
  mutate(Year = format(datetime, format="%Y"),
         ...
         Year.Month = format(datetime, format="%Y-%m"),
         Year.Month.Day = format(datetime, format="%Y-%m-%d"),
         Hour = str_replace_na(Hour),
         Hour = case_when(Hour == "NA" ~ "09:00:00", TRUE ~ Hour))
...
###### [EOL 2 : Table of time series anomalies regression analyses (2014-2022) - EOL temperature & salinity]{style="color:green"}  
...
**EOL 2 : Plot of anomalies : Temperature (°C) (2014-2022) - EOL**  
...
plot_ano_temp_EOL_2 <- ggplot(data = anomalies_EOL_2, aes(x = datetime, y = Temperature_ano), na.rm=TRUE) +
  ...
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="EOL - Temperature anomalies + trend (2014-2022)",x="", y="Temp. (°C)") +
  annotate(geom="text", x=as.POSIXct("2015-12-03 01:00:00"), y=5, label="slope : 0.071 ± 0.008 *", color="black")
...
**EOL 2 : Plot of anomalies : Salinity (2014-2022) - EOL**  
...
plot_ano_sal_EOL_2 <- ggplot(data = anomalies_EOL_2, aes(x = datetime, y = Salinity_ano), na.rm=TRUE) +
  ...
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="EOL - Salinity anomalies + trend (2014-2022)",x="", y="Temp. (°C)") +
  annotate(geom="text", x=as.POSIXct("2015-12-03 01:00:00"), y=1, label="slope : 0.0019 ± 0.0013 (non-significative)", color="black")
...
#### Part 4 : Trend analysis by residuals : hourly dataset jan.2018 - dec.2022  
  
  

```{r importation Data EOL 3 - hourly, echo=FALSE, warning=FALSE, message=FALSE}

EOL_daily_3 <- read_delim("../Data/EOL_18-23_hourly.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE) %>% 
  rename(temperature=temp_eol_c, salinity=sal_eol_psu) %>% 
  mutate(datetime = as.POSIXct(datetime)) %>% 
...
###### [EOL 3 : Table and plot of monthly means (2018-2022) - EOL temperature & salinity]{style="color:blue"}  
  
Missing month : April 2019 (imputation by somlit data)  
  

```{r EOL 3 - calcul monthly means - temperature & salinity, echo=TRUE, warning=FALSE, message=FALSE}

##merge with EOL_3 :
year_2019 <- year_2019 %>% mutate(datetime = as.POSIXct(datetime))

EOL_daily_3 <- full_join(EOL_daily_3, year_2019, by=c("datetime", "temperature"="impute_temp", "salinity"="impute_sal"))

EOL_daily_3 <- EOL_daily_3 %>% arrange(datetime)


#MAJ EOL variables :
EOL_daily_3 <- EOL_daily_3 %>% 
  mutate(Year = format(datetime, format="%Y"),
         Month = format(datetime, format="%m"),
         Year.Month = format(datetime, format="%Y-%m"),
         Year.Month.Day = format(datetime, format="%Y-%m-%d"),
...
###### [EOL 3 : Table of time series anomalies regression analyses (2018-2022) - EOL temperature & salinity]{style="color:green"}  
...
**EOL 3 : Plot of anomalies : Temperature (°C) (2018-2022) - EOL**  
...
plot_ano_temp_EOL_3 <- ggplot(data = anomalies_EOL_3, aes(x = datetime, y = Temperature_ano), na.rm=TRUE) +
  ...
  geom_point(colour="blue", na.rm=TRUE, size=0.65) + 
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="EOL - Temperature anomalies + trend (2018-2022)",x="", y="Temp. (°C)") +
...
**EOL 3 : Plot of anomalies : Salinity (2018-2022) - EOL**  
...
plot_ano_sal_EOL_3 <- ggplot(data = anomalies_EOL_3, aes(x = datetime, y = Salinity_ano), na.rm=TRUE) +
  ...
  geom_point(colour="blue", na.rm=TRUE, size=0.65) + 
  geom_smooth(method=lm, colour="black", fill="grey", linewidth=0.6, na.rm=TRUE) +
  labs(title="EOL - Salinity anomalies + trend (2018-2022)",x="", y="Temp. (°C)") +
  annotate(geom="text", x=as.POSIXct("2019-03-16 01:00:00"), y=1, label="slope : 0.019 ± 0.0008 *", color="black")
...
```{r Summary table of regression analyses on residuals, echo=FALSE, warning=FALSE, message=FALSE}
...
#plot EOL
 Wind_EOL %>% 
   ...
   ggtitle("Wind speed distribution - EOL buoy (2009-2022)") + 
...
```{r model + Cap-Ferrat lines - means per day, echo=FALSE, warning=FALSE, message=FALSE}
...
winds_mean_plot <- winds_mean_plot_long %>% 
  ggplot() +
  ggtitle("EOL and Cap-Ferrat winds (means per day) : Dec.2020 - Jan.2022") +
  geom_line(aes(x=date, y=value, color= Legend), size=0.7) +
  scale_y_continuous(name="Wind speed (m/s)") + 
  scale_x_datetime(name="") + 
  scale_colour_discrete(type=c("#69b3a2", "#CC5500"))
