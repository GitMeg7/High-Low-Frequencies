...
par_periode <- c("2020-10-29 12:20:00", "2020-12-09 08:00:00","2020-12-17 10:00:00","2021-03-16 11:00:00","2021-03-17 11:00:00", "2021-05-21 06:50:00","2021-06-11 10:00:00", "2021-11-26 09:00:00", "2021-12-03 11:00:00", "2022-03-23 08:50:00", "2022-09-02 08:40:00")
...
dygraph(par_xts, ylab = "") %>%
    ...
    dyEvent(as.POSIXct("2020-12-28 09:10:00"), "par_10m stops", labelLoc = "top", color="black",strokePattern = "dashed") %>% 
      dyEvent(as.POSIXct("2021-10-03 03:15:00"), "dirty - data removing", labelLoc = "top", color="black",strokePattern = "dashed") %>% 
  ...
  dyEvent(as.POSIXct("2021-03-30 09:50:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
    dyEvent(as.POSIXct("2021-04-21 08:20:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
      dyEvent(as.POSIXct("2021-06-11 10:00:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
      ...
      dyEvent(as.POSIXct("2021-08-17 08:00:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>% 
        ...
        dyEvent(as.POSIXct("2021-10-19 08:30:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>%
            dyEvent(as.POSIXct("2021-12-03 11:00:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>%

        dyEvent(as.POSIXct("2022-02-23 09:30:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>%
          dyEvent(as.POSIXct("2022-03-07 09:30:00"), "cleaning", labelLoc = "top", color="red",strokePattern = "dashed") %>%
...
dyShading(from = par_periode[1], to = par_periode[2], color = "#C0C0C0") %>%
  ...
  dyShading(from = par_periode[5], to = par_periode[6], color = "#C0C0C0") %>%
   ...
   dyShading(from = par_periode[9], to = par_periode[10], color = "#C0C0C0") %>%
      dyShading(from = par_periode[10], to = par_periode[11], color = "#E0E0E0") %>%
...
par_periode_air <- c("2020-12-17 10:00:00","2021-03-16 09:00:00","2021-03-17 13:00:00","2021-06-11 07:30:00", "2021-06-11 09:00:00","2021-11-26 09:00:00","2021-12-03 11:00:00", "2022-03-23 08:50:00", "2022-09-02 08:50:00")
####### AIR
...
kbl(par_log)%>%
  ...
  row_spec(c(1:23, 47:69), bold = F, color = "white", background = "#97C7FA") %>% 
...
<!-- dySeries("ph_s_sf_temp_insi", drawPoints = TRUE, pointSize=6, label = "spectro", color="green")%>% -->
...
```{r sami pH data, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
# read deployment file
...
```{r sami pH, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
 ...
 spectro <- read.table( "../../pCloud Sync/exp153_carbonates_point_B/output_jp/output/output_somlit.csv", header = T, dec = ".", as.is = T, sep = ",", fill = TRUE) %>%
  dplyr::filter(depth == 0)
 spectro$datetime <- as.POSIXct(spectro$sampling_date, format="%Y-%m-%dT%H:%M:%SZ",tz="UTC" )
# put 09:00:00 as time (local time)
spectro <- spectro %>%
  dplyr::mutate(time = " 09:00:00") %>%
  dplyr::mutate(datetime = paste0(datetime, time)) 
spectro$datetime <- ymd_hms(spectro$datetime, tz="Europe/Paris")

# convert local time to UTC time
spectro$datetime <- with_tz(spectro$datetime, "UTC")#  tidyr::unite("datetime", sampling_date, sampling_hour, sep=" ", remove =T) %>%  
 ...
 #                    temp_field = mean(temp_field, na.rm = TRUE) 
 #                     ) 
 # add spectro T insi to seaFET data  
 #ph_sf <- left_join(ph_sf, spectro_mean%>%dplyr::select(datetime,ph_s_sf_temp_insi, temp_field,salinity_field),by="datetime") 
 # ph_sf <- left_join(ph_sf, spectro%>%dplyr::select(datetime,ph_spec_insitu, temperature ,salinity),by="datetime") 




## add spectro to sami data
sami <- left_join(sami, spectro%>%dplyr::select(datetime,pH_spec_insitu),by="datetime")
## Add event periodes manually
## Events
sami_events <- c("2019-12-16 11:00:00", "2020-02-25 16:00:00","2020-06-17 08:00:00","2020-06-19 10:00:00","2020-07-15 10:00:00","2020-07-17 18:00:00", "2020-08-10 00:00:00", "2020-12-10 13:00:00","2020-12-17 09:00:00","2021-02-17 09:00:00","2021-03-17 12:00:00","2021-03-30 09:30:00","2021-04-22 12:00:00","2021-06-11 08:00:00","2021-07-20 07:00:00","2021-09-24 13:00:00","2021-10-19 10:00:00", "2021-11-26 08:00:00","2021-11-26 10:00:00", "2022-01-18 09:00:00", "2022-02-17 12:00:00", "2022-03-23 09:00:00","2022-05-09 09:00:00","2022-05-12 11:00:00","2022-09-02 07:00:00")

## PLOT
sami_xts <- dplyr::select(sami, datetime, ph_sami, temp_int_sami, temp_ext_sami,pH_spec_insitu)
...
presAnnotation <- function(dygraph, x, text) {
 ...
 dygraph(sami_xts, ylab = "") %>%
  ...
  dySeries(axis = "y2","temp_int_sami", drawPoints = TRUE, pointSize=2, label = "temp_int_sami") %>%
     dySeries("pH_spec_insitu", drawPoints = TRUE, pointSize=4, label = "spectro", color = "red")%>%

  dySeries(axis = "y2", "temp_ext_sami", drawPoints = TRUE, pointSize=2, label = "temp_ext_sami") %>%

   dyEvent(as.POSIXct("2020-07-16 12:00:00", tz="GMT"), "Reagent bag replacement", labelLoc = "bottom", color="red") %>%
   ...
   dyEvent(as.POSIXct("2021-04-04 10:00:00", tz="GMT"), "SAMI stops", labelLoc = "top", color="black") %>%
    dyEvent(as.POSIXct("2021-09-24 13:00:00", tz="GMT"), "iSAMI in use", labelLoc = "bottom", color="red") %>%
 dyEvent(as.POSIXct("2022-01-27 19:00:00", tz="GMT"), "TRIS calib", labelLoc = "bottom", color="red") %>%
    dyEvent(as.POSIXct("2022-02-14 10:00:00", tz="GMT"), "Battery replacement", labelLoc = "bottom", color="red") %>%
   dyEvent(as.POSIXct("2022-09-12 08:00:00", tz="GMT"), "Reagent bag replacement", labelLoc = "bottom", color="red") %>%

    dyEvent(as.POSIXct("2022-09-12 18:00:00", tz="GMT"), "TRIS calib", labelLoc = "bottom", color="red") %>%

  dyShading(from = sami_events[1], to = sami_events[2], color = "#B7B7B7") %>%
  dyShading(from = sami_events[2], to = sami_events[3], color = "#E0E0E0") %>%
   dyShading(from = sami_events[4], to = sami_events[5], color = "#B7B7B7") %>%
    dyShading(from = sami_events[5], to = sami_events[6], color = "#E0E0E0") %>%
    dyShading(from = sami_events[6], to = sami_events[7], color = "#B7B7B7") %>%
    dyShading(from = sami_events[8], to = sami_events[9], color = "#E0E0E0") %>%
    dyShading(from = sami_events[9], to = sami_events[10], color = "#B7B7B7") %>%
 dyShading(from = sami_events[11], to = sami_events[12], color = "#E0E0E0") %>%
    dyShading(from = sami_events[12], to = sami_events[13], color = "#B7B7B7") %>%
       dyShading(from = sami_events[13], to = sami_events[14], color = "#E0E0E0") %>%
    dyShading(from = sami_events[14], to = sami_events[15], color = "#B7B7B7") %>%
   dyShading(from = sami_events[16], to = sami_events[17], color = "#E0E0E0") %>%
          dyShading(from = sami_events[17], to = sami_events[18], color = "#B7B7B7") %>%
      dyShading(from = sami_events[19], to = sami_events[20], color = "#E0E0E0") %>%
     dyShading(from = sami_events[21], to = sami_events[22], color = "#B7B7B7") %>%
         dyShading(from = sami_events[22], to = sami_events[23], color = "#E0E0E0") %>%
   dyShading(from = sami_events[24], to = sami_events[25], color = "#B7B7B7") %>%
...
```{r kable log sami}
sami_log <- read_excel(paste0(path_sami, "../../Sami_SeaFet_log_metadata_vlfr.xlsx"), sheet = 2)
...
kbl(sami_log)%>%
    kable_paper() %>% 
    kable_styling(fixed_thead = T)%>%
  row_spec(c(2:3, 7:8, 12:15, 17:19, 21:26,30:32, 35), bold = T, color = "white", background = "#97C7FA") %>%     scroll_box(width = "100%", height = "200px")
...
eol <- rbind(eol, eol_4)
eol <- dplyr::mutate(eol, datetime=str_c(Date, " ", Heure) %>% parse_datetime(format="%Y%m%d %H%M%S"))
# round time to closest 30 mins
eol$datetime <- round_date(eol$datetime, "30 mins")
...
# # calculate time difference bettween 2 rows
# #options(scipen = 0) 
# eol <- eol%>%
# dplyr::mutate(diff_time = as.numeric(as.numeric(datetime - lag(datetime))/60))
# 
# diff_eol_xts <- dplyr::select(eol, datetime, diff_time)
# diff_eol_xts <- as.xts(diff_eol_xts, order.by=diff_eol_xts$datetime)
# dygraph(diff_eol_xts, main = "", ylab = "") %>%
#   dySeries("diff_time", drawPoints = TRUE, pointSize=2, label = "Temp")%>%
...
### SAVE
saveRDS(rh_B_Bplus_1m_mean, file = paste0(path, "data/eol/rh_B_Bplus_1m_mean.rds"))
```
...
### SAVE
saveRDS(rh_B_Bplus_50m_mean, file = paste0(path, "data/eol/rh_B_Bplus_50m_mean.rds"))
```
...
# Correlation is OK
#Take B+ as data and put B if gap.
rh_B_Bplus_1m_mean <- rh_B_Bplus_1m_mean %>%
dplyr::mutate(rh_B_Bplus_1m_mean, mean_temp_rhBplus_B = case_when(is.na(mean_temp_rhBplus) ~ mean_temp_rhB,
                                                                 TRUE ~ mean_temp_rhBplus))
### SAVE
saveRDS(rh_B_Bplus_1m_mean, file = paste0(path, "data/eol/rh_B_Bplus_1m_mean.rds"))
```
...
#Take B+ as data and put B if gap.
rh_B_Bplus_50m_mean <- rh_B_Bplus_50m_mean %>%
dplyr::mutate(rh_B_Bplus_50m_mean, mean_temp_rhBplus_B = case_when(is.na(mean_temp_rhBplus) ~ mean_temp_rhB,
                                                                 TRUE ~ mean_temp_rhBplus))
### SAVE
saveRDS(rh_B_Bplus_50m_mean, file = paste0(path, "data/eol/rh_B_Bplus_50m_mean.rds"))
```
...
# Correlation is OK
rh_B_Bplus_1m_mean <- rh_B_Bplus_1m_mean %>%
dplyr::mutate(rh_B_Bplus_1m_mean, mean_sal_rhBplus_B = case_when(is.na(mean_sal_rhBplus) ~ mean_sal_rhB,
                                                                 TRUE ~ mean_sal_rhBplus))
### SAVE
saveRDS(rh_B_Bplus_1m_mean, file = paste0(path, "data/eol/rh_B_Bplus_1m_mean.rds"))
...
#Take B+ as data and put B if gap.
rh_B_Bplus_50m_mean <- rh_B_Bplus_50m_mean %>%
dplyr::mutate(rh_B_Bplus_50m_mean, mean_sal_rhBplus_B = case_when(is.na(mean_sal_rhBplus) ~ mean_sal_rhB,
                                                                 TRUE ~ mean_sal_rhBplus))
### SAVE
saveRDS(rh_B_Bplus_50m_mean, file = paste0(path, "data/eol/rh_B_Bplus_50m_mean.rds"))
```
...
#Take B+ as data and put B if gap.
rh_B_Bplus_1m_mean <- rh_B_Bplus_1m_mean %>%
dplyr::mutate(rh_B_Bplus_1m_mean, mean_oxy_mll_rhBplus_B = case_when(is.na(mean_oxy_mll_rhBplus) ~ mean_oxy_mll_rhB,
                                                                 TRUE ~ mean_oxy_mll_rhBplus))
### SAVE
saveRDS(rh_B_Bplus_1m_mean, file = paste0(path, "data/eol/rh_B_Bplus_1m_mean.rds"))
```
...
winkler_surface<- tbl_df(
  read_delim(file=paste0(path,"/data/Somlit/Somlit_Extraction_hydro_surface_1997_2022.csv"),
delim = ";", 
skip= 4,
...
#Winkler FOND
winkler_fond<- tbl_df(
  read_delim(file=paste0(path,"/data/Somlit/Somlit_Extraction_hydro_fond_1997_2022.csv"), 
delim = ";", 
skip= 4,
col_names = FALSE,# col_types = typ,
na = c("NA", "999999")) )#%>%
...
rh_B_Bplus_1m_mean <- full_join(rh_B_Bplus_1m_mean, wink_1m, by="datetime")

# ###
# ptb_wink_ctd_xts <- dplyr::select(rh_B_Bplus_1m_mean, date, mean_oxy_mll_rhBplus,mean_oxy_mll_rhB, mean_oxy_mll_rhBplus_B)
...
# dySeries("mean_oxy_mll_rhB", drawPoints = TRUE, pointSize=3, label = "Oxy PTB", color ="green")%>%
#   dySeries("mean_oxy_mll_rhBplus_B", drawPoints = TRUE, pointSize=3, label = "Oxy PTB + PTB+", color ="black")%>%
# 
#  # dyLegend(show = "always", width = 600)%>%
#   #dyAxis("y",valueRange = c(-0.1, 0.11))%>%
# dyOptions(
...
ptb_wink_ctd_xts <- dplyr::select(rh_B_Bplus_1m_mean, datetime, mean_oxy_mll_rhBplus_B, ptb_oxy_wink)
ptb_wink_ctd_xts <- as.xts(ptb_wink_ctd_xts, order.by=ptb_wink_ctd_xts$datetime)

dygraph(ptb_wink_ctd_xts,  main = "Oxygen (ml/l) 1m: PTB+ + Winkler", ylab = "") %>%
dySeries("mean_oxy_mll_rhBplus_B", drawPoints = TRUE, pointSize=4, label = "Oxy PTB+/B", color ="blue") %>%
...
#delta oxy
rh_B_Bplus_1m_mean <- rh_B_Bplus_1m_mean %>% 
  dplyr::mutate(oxy_mll_rhBplus_B_wink_detla = mean_oxy_mll_rhBplus_B - ptb_oxy_wink, 
                oxy_mll_rhB_wink_detla = mean_oxy_mll_rhB - ptb_oxy_wink, 
                oxy_mll_rhBplus_wink_detla = mean_oxy_mll_rhBplus - ptb_oxy_wink)
#plot
oxy_rhBplus_wink_xts <- dplyr::select(rh_B_Bplus_1m_mean,datetime, oxy_mll_rhBplus_B_wink_detla)
...
dygraph(oxy_rhBplus_wink_xts,  main = "Delta Oxy ptB+ Winkler 1m", ylab = "") %>%
     dySeries("oxy_mll_rhBplus_B_wink_detla", drawPoints = TRUE, pointSize=2, label = "delta (ml/l)", color="black")%>%
   dyLimit(0,strokePattern = "dashed", color = "black") %>% 
 # dyLegend(show = "always", width = 600)%>%
 dyAxis("y",valueRange = c(-1.8, 1.3))%>%
dyOptions(
    drawGrid = TRUE,
    drawPoints = TRUE,
    useDataTimezone = TRUE)%>%
...
fit <- lmodel2(data = rh_B_Bplus_1m_mean, ptb_oxy_wink ~ mean_oxy_mll_rhBplus_B, nperm = 99)
p_Oxyptbwink_Oxyptb_1m <-  ggreg2(fit, "mean_oxy_mll_rhBplus_B", "ptb_oxy_wink") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
  mytheme+labs(x="Oxy Point B+ CTD", y="Oxy Point B Winkler")
print(p_Oxyptbwink_Oxyptb_1m)
...
## Oxy umolkg 50m
rh_oxymll_xts <- dplyr::select(rh_B_Bplus_50m_mean, datetime, mean_oxy_mll_rhBplus,mean_oxy_mll_rhB)
rh_oxymll_xts <- as.xts(rh_oxymll_xts, order.by=rh_oxymll_xts$datetime)
...
#Take B+ as data and put B if gap.
rh_B_Bplus_50m_mean <- rh_B_Bplus_50m_mean %>%
dplyr::mutate(rh_B_Bplus_50m_mean, mean_oxy_mll_rhBplus_B = case_when(is.na(mean_oxy_mll_rhBplus) ~ mean_oxy_mll_rhB,
                                                                 TRUE ~ mean_oxy_mll_rhBplus))
### SAVE
saveRDS(rh_B_Bplus_50m_mean, file = paste0(path, "data/eol/rh_B_Bplus_50m_mean.rds"))
```
...
rh_B_Bplus_50m_mean <- full_join(rh_B_Bplus_50m_mean, wink_50m, by="datetime")


###
# PLOT all oxygen data as time series
ptb_wink_ctd_xts <- dplyr::select(rh_B_Bplus_50m_mean, datetime, mean_oxy_mll_rhBplus_B, ptb_oxy_wink)
ptb_wink_ctd_xts <- as.xts(ptb_wink_ctd_xts, order.by=ptb_wink_ctd_xts$datetime)
...
rh_B_Bplus_50m_mean <- rh_B_Bplus_50m_mean %>% 
  dplyr::mutate(oxy_mll_rhBplus_wink_detla = mean_oxy_mll_rhBplus - ptb_oxy_wink, 
                oxy_mll_rhB_wink_detla = mean_oxy_mll_rhB - ptb_oxy_wink,
                oxy_mll_rhBplus_B_wink_detla = mean_oxy_mll_rhBplus_B - ptb_oxy_wink)
#plot
oxy_rhBplus_wink_xts <- dplyr::select(rh_B_Bplus_50m_mean,datetime, oxy_mll_rhBplus_B_wink_detla)
oxy_rhBplus_wink_xts <- as.xts(oxy_rhBplus_wink_xts, order.by=oxy_rhBplus_wink_xts$datetime)

dygraph(oxy_rhBplus_wink_xts,  main = "Delta Oxy ptB+ Winkler 50m", ylab = "") %>%
     dySeries("oxy_mll_rhBplus_B_wink_detla", drawPoints = TRUE, pointSize=2, label = "delta Oxy ml/l", color="black")%>%
   dyLimit(0,strokePattern = "dashed", color = "black") %>% 
 # dyLegend(show = "always", width = 600)%>%
   dyAxis("y",valueRange = c(-1.6, 1.4))%>%
dyOptions(
  ...
  dyRangeSelector(height = 30)
###
titre <- "Oxygen 50m: Point B+/B CTD vs Point B Winkler"
fit <- lmodel2(data = rh_B_Bplus_50m_mean, ptb_oxy_wink ~ mean_oxy_mll_rhBplus_B, nperm = 99)
p_Oxyptbwink_Oxyptb_50m <-  ggreg2(fit, "mean_oxy_mll_rhBplus_B", "ptb_oxy_wink") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
  mytheme+labs(x="Oxy Point B+/B CTD", y="Oxy Point B Winkler")
print(p_Oxyptbwink_Oxyptb_50m)
...
#eol_ptb_1m
eol_ptb_temp_xts <- dplyr::select(combined1, datetime, mean_temp_rhBplus_B, temp_eol)
...
dySeries("temp_eol", drawPoints = TRUE, pointSize=3, label = "Temp EOL", color ="red")%>%
  dySeries("mean_temp_rhBplus_B", drawPoints = TRUE, pointSize=2, label = "Temp PTB+", color ="blue") %>%
...
fit <- lmodel2(data = combined1, mean_temp_rhBplus_B ~ temp_eol, nperm = 99)
p_Tptb_Teol <-  ggreg2(fit, "temp_eol", "mean_temp_rhBplus_B") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
...
eol_ptb_sal_xts <-  combined1%>%
  dplyr::filter((!is.na(combined1$mean_sal_rhBplus_B)))%>%
  dplyr::select(datetime, mean_sal_rhBplus_B, sal_eol)
...
dygraph(eol_ptb_sal_xts,  main = "Salinity comparaison: EOL - ptB+", ylab = "") %>%
    dySeries("mean_sal_rhBplus_B", drawPoints = TRUE, pointSize=4, label = "Sal ptB+", color ="blue") %>%
...
p_Sptb_Seol <-  ggreg2(fit, "sal_eol", "mean_sal_rhBplus_B") +
  geom_abline(aes(slope= 1, intercept =0), linetype = 2) +
...
#delta sal
combined1 <- combined1 %>% 
  dplyr::mutate(sal_eol_rhBplus_detla = mean_sal_rhBplus_B - sal_eol)
...
dyShading(from =  sal_periode[17], to= sal_periode[18], color = "#B7B7B7") %>%
  dyShading(from =  sal_periode[19], to= sal_periode[20], color = "#B7B7B7") %>%
...
Drifting periodes in grey have been selected and a trend with time has been exctracted. The parameters of the trend have been used to correct (green) EOL data  according to SOMLIT data taken as reference.
...
#Select drifting periodes
p8 <- combined1%>%
  dplyr::select(datetime, mean_sal_rhBplus_B, sal_eol,sal_eol_rhBplus_detla )%>%
    dplyr::filter(sal_eol_rhBplus_detla > -0.3 & sal_eol_rhBplus_detla < 0.3 )%>%
  dplyr::filter(datetime >"2014-10-14 00:00:00" & datetime <"2015-06-16 23:59:00" )

## Regression lineaire avec le temps
fit_p8 <- lm(data = p8, sal_eol_rhBplus_detla ~ unclass(datetime) )
slope_p8 <- fit_p8$coefficients[[2]]
intercept_p8 <- fit_p8$coefficients[[1]]

p8_plot <-  ggplot(data=p8, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p8, intercept =intercept_p8), linetype = 1, col="red") +ggtitle("p8, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3")+
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(p8_plot)

#correction des valeurs EOL avec la regression
p8 <- p8%>%
  dplyr::mutate(correction = slope_p8 * unclass(datetime) + intercept_p8,
                sal_eol_corr = sal_eol + correction )

# plot
 p8_corr_xts <-p8%>%
   dplyr::select(datetime, mean_sal_rhBplus_B, sal_eol, sal_eol_corr)

p8_corr_xts <- as.xts(p8_corr_xts, order.by=p8_corr_xts$datetime)

dygraph(p8_corr_xts,  main = "p8, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("mean_sal_rhBplus_B", drawPoints = TRUE, pointSize=2, label = "sal ptB+", color="blue")%>%
       dySeries("sal_eol", drawPoints = TRUE, pointSize=2, label = "sal EOL", color="red")%>%
       dySeries("sal_eol_corr", drawPoints = TRUE, pointSize=2, label = "sal EOL corr", color="green")%>%
dyOptions(
    drawGrid = TRUE,
...
p9 <- combined1%>%
  dplyr::select(datetime, mean_sal_rhBplus_B, sal_eol,sal_eol_rhBplus_detla )%>%
    dplyr::filter(sal_eol_rhBplus_detla > -0.3 & sal_eol_rhBplus_detla < 0.3 )%>%
  dplyr::filter(datetime >"2020-11-24 00:00:00" & datetime <"2021-03-09 23:59:00" )

## Regression lineaire avec le temps
fit_p9 <- lm(data = p9, sal_eol_rhBplus_detla ~ unclass(datetime) )
slope_p9 <- fit_p9$coefficients[[2]]
intercept_p9 <- fit_p9$coefficients[[1]]


p9_plot <-  ggplot(data=p9, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p9, intercept =intercept_p9), linetype = 1, col="red") + ggtitle("p9, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3")+
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(p9_plot)

#correction des valeurs EOL avec la regression
p9 <- p9%>%
  dplyr::mutate(correction = slope_p9 * unclass(datetime) + intercept_p9,
                sal_eol_corr = sal_eol + correction )
...
p9_corr_xts <- as.xts(p9_corr_xts, order.by=p9_corr_xts$datetime)

dygraph(p9_corr_xts,  main = "p9, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("mean_sal_rhBplus_B", drawPoints = TRUE, pointSize=2, label = "sal ptB+", color="blue")%>%
       dySeries("sal_eol", drawPoints = TRUE, pointSize=2, label = "sal EOL", color="red")%>%
       dySeries("sal_eol_corr", drawPoints = TRUE, pointSize=2, label = "sal EOL corr", color="green")%>%
dyOptions(
...
p10 <- combined1%>%
  dplyr::select(datetime, mean_sal_rhBplus_B, sal_eol,sal_eol_rhBplus_detla )%>%
    dplyr::filter(sal_eol_rhBplus_detla > -0.3 & sal_eol_rhBplus_detla < 0.3 )%>%
  dplyr::filter(datetime >"2021-06-29 00:00:00" & datetime <"2021-08-03 23:59:00" )

## Regression lineaire avec le temps
fit_p10 <- lm(data = p10, sal_eol_rhBplus_detla ~ unclass(datetime) )
slope_p10 <- fit_p10$coefficients[[2]]
intercept_p10 <- fit_p10$coefficients[[1]]


p10_plot <-  ggplot(data=p10, aes(x=datetime, y=sal_eol_rhBplus_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_p10, intercept =intercept_p10), linetype = 1, col="red") + ggtitle("p10, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3")+
  mytheme+labs(y="delta Sal EOL/PtB", x="") + scale_x_datetime(date_breaks="month",  labels=date_format("%b %Y")) + theme(axis.text.x = element_text(angle = 100, vjust = 1, hjust=1))
print(p10_plot)

#correction des valeurs EOL avec la regression
p10 <- p10%>%
  dplyr::mutate(correction = slope_p10 * unclass(datetime) + intercept_p10,
                sal_eol_corr = sal_eol + correction )
...
p10_corr_xts <- as.xts(p10_corr_xts, order.by=p10_corr_xts$datetime)

dygraph(p10_corr_xts,  main = "p10, delta Sal EOL / ptB+ 1m, data with delta -0.3/+0.3", ylab = "") %>%
     dySeries("mean_sal_rhBplus_B", drawPoints = TRUE, pointSize=2, label = "sal ptB+", color="blue")%>%
       dySeries("sal_eol", drawPoints = TRUE, pointSize=2, label = "sal EOL", color="red")%>%
       dySeries("sal_eol_corr", drawPoints = TRUE, pointSize=2, label = "sal EOL corr", color="green")%>%
dyOptions(
...
combined1 <- combined1%>%
  dplyr::mutate(correction_sal = case_when(datetime >"2014-05-06 00:00:00" & datetime <"2014-10-07 23:59:00" ~ slope_p1 * unclass(datetime) + intercept_p1, 
                                       ...
                                       datetime >"2014-10-14 00:00:00" & datetime <"2015-06-16 23:59:00" ~ slope_p8 * unclass(datetime) + intercept_p8,
                                       datetime >"2020-11-24 00:00:00" & datetime <"2021-03-09 23:59:00" ~ slope_p9 * unclass(datetime) + intercept_p9,
                                       datetime >"2021-06-29 00:00:00" & datetime <"2021-08-03 23:59:00" ~ slope_p10 * unclass(datetime) + intercept_p10,TRUE ~ 0),
                slope_sal = case_when(datetime >"2014-05-06 00:00:00" & datetime <"2014-10-07 23:59:00" ~ slope_p1 , 
                                       datetime >"2015-06-22 00:00:00" & datetime <"2016-03-22 23:59:00" ~ slope_p2 ,
                                       datetime >"2017-01-10 00:00:00" & datetime <"2017-07-04 23:59:00" ~ slope_p3 ,
                                       datetime >"2017-11-07 00:00:00" & datetime <"2019-03-19 23:59:00" ~ slope_p4 ,
                                       datetime >"2019-07-09 00:00:00" & datetime <"2020-03-10 23:59:00" ~ slope_p5 ,
                                       datetime >"2020-04-23 00:00:00" & datetime <"2020-06-30 23:59:00" ~ slope_p6 ,
                                       datetime >"2020-07-21 00:00:00" & datetime <"2020-11-17 23:59:00" ~ slope_p7 ,
                                       datetime >"2014-10-14 00:00:00" & datetime <"2015-06-16 23:59:00" ~ slope_p8 ,
                                       datetime >"2020-11-24 00:00:00" & datetime <"2021-03-09 23:59:00" ~ slope_p9 ,
                                       datetime >"2021-06-29 00:00:00" & datetime <"2021-08-03 23:59:00" ~ slope_p10 ,TRUE ~ NA_real_),
                 intercept_sal = case_when(datetime >"2014-05-06 00:00:00" & datetime <"2014-10-07 23:59:00" ~ intercept_p1 , 
                                       datetime >"2015-06-22 00:00:00" & datetime <"2016-03-22 23:59:00" ~ intercept_p2 ,
                                       datetime >"2017-01-10 00:00:00" & datetime <"2017-07-04 23:59:00" ~ intercept_p3 ,
                                       datetime >"2017-11-07 00:00:00" & datetime <"2019-03-19 23:59:00" ~ intercept_p4 ,
                                       datetime >"2019-07-09 00:00:00" & datetime <"2020-03-10 23:59:00" ~ intercept_p5 ,
                                       datetime >"2020-04-23 00:00:00" & datetime <"2020-06-30 23:59:00" ~ intercept_p6 ,
                                       datetime >"2020-07-21 00:00:00" & datetime <"2020-11-17 23:59:00" ~ intercept_p7 ,
                                       datetime >"2014-10-14 00:00:00" & datetime <"2015-06-16 23:59:00" ~ intercept_p8 ,
                                       datetime >"2020-11-24 00:00:00" & datetime <"2021-03-09 23:59:00" ~ intercept_p9 ,
                                       datetime >"2021-06-29 00:00:00" & datetime <"2021-08-03 23:59:00" ~ intercept_p10 ,TRUE ~ NA_real_),
                 periode_sal = case_when(datetime >"2014-05-06 00:00:00" & datetime <"2014-10-07 23:59:00" ~ "p1" , 
                                       datetime >"2015-06-22 00:00:00" & datetime <"2016-03-22 23:59:00" ~ "p3" ,
                                       datetime >"2017-01-10 00:00:00" & datetime <"2017-07-04 23:59:00" ~ "p4" ,
                                       datetime >"2017-11-07 00:00:00" & datetime <"2019-03-19 23:59:00" ~ "p5" ,
                                       datetime >"2019-07-09 00:00:00" & datetime <"2020-03-10 23:59:00" ~ "p6" ,
                                       datetime >"2020-04-23 00:00:00" & datetime <"2020-06-30 23:59:00" ~ "p7" ,
                                       datetime >"2020-07-21 00:00:00" & datetime <"2020-11-17 23:59:00" ~ "p8" ,
                                       datetime >"2014-10-14 00:00:00" & datetime <"2015-06-16 23:59:00" ~ "p2" ,
                                       datetime >"2020-11-24 00:00:00" & datetime <"2021-03-09 23:59:00" ~ "p9" ,
                                       datetime >"2021-06-29 00:00:00" & datetime <"2021-08-03 23:59:00" ~ "p10" ,TRUE ~ NA_character_),
                sal_eol_corr = sal_eol + correction_sal)
### SAVE
saveRDS(combined1, file = paste0(path, "data/eol/combined_eol_rh_data.rds"))

## PLOT LOW FREQUENCY
eol_corr_xts <- dplyr::select(combined1, datetime, sal_eol, sal_eol_corr)
eol_corr_xts <- as.xts(eol_corr_xts, order.by=eol_corr_xts$datetime)

dygraph(eol_corr_xts, main = "EOL sal (LOW Frequency)", ylab = "") %>%
...
## **Correction of High Frequency salinity data**
...
dygraph(eol_corr_xts, main = "EOL sal (HIGH FREQUENCY)", ylab = "") %>%
  dySeries("sal_eol_corr", drawPoints = TRUE, pointSize=2,  color = "green", label = "sal EOL corr")%>%
  dySeries("sal_eol", drawPoints = TRUE, pointSize=2, color = "blue",label = "sal EOL") %>%
dyShading(from =  sal_periode[1], to= sal_periode[2], color = "#B7B7B7") %>%
dyShading(from =  sal_periode[3], to= sal_periode[4], color = "#B7B7B7") %>%
...
###
# PLOT all oxygen data as time series
eol_ptb_oxy_xts <- dplyr::select(combined1, datetime, mean_oxy_mll_rhBplus_B, oxy_eol, ptb_oxy_wink)
...
dySeries("mean_oxy_mll_rhBplus_B", drawPoints = TRUE, pointSize=4, label = "Oxy ptB+", color ="blue") %>%
...
fit <- lmodel2(data = combined1, oxy_eol ~ ptb_oxy_wink, nperm = 99)
...
#delta oxy
combined1 <- combined1 %>% 
  dplyr::mutate(oxy_mll_Bplus_wink_detla = ptb_oxy_wink - mean_oxy_mll_rhBplus_B,
                oxy_mll_eol_wink_detla =   ptb_oxy_wink - oxy_eol)

oxy_periode <- c("2017-07-11 00:00:00","2021-07-06 23:59:00","2021-07-13 00:00:00","2022-06-21 23:59:00")

#plot
oxy_eol_wink_xts <- dplyr::select(combined1,datetime, oxy_mll_eol_wink_detla)
...
dyShading(from =  oxy_periode[1], to= oxy_periode[2], color = "#B7B7B7") %>%
dyShading(from =  oxy_periode[3], to= oxy_periode[4], color = "#B7B7B7") %>%
dyShading(from =  oxy_periode[5], to= oxy_periode[6], color = "#B7B7B7") %>%
...
Drifting periodes in grey have been selected and a trend with time has been exctracted. The parameters of the trend have been used to correct (orange) EOL data (red) according to SOMLIT Winkler data taken as reference (green).

```{r eol pointb comp Oxy periodes, echo=TRUE, message=FALSE, warning=FALSE, out.width='100%'}
##### p1 wink ####
#Select drifting periodes
p1wink <- combined1%>%
  dplyr::select(datetime, mean_oxy_mll_rhBplus_B, oxy_eol, oxy_mll_eol_wink_detla ,ptb_oxy_wink )%>%
    #dplyr::filter(oxy_mll_eol_wink_detla > -0.3 & oxy_mll_eol_wink_detla < 0.3 )%>%
  dplyr::filter(datetime >"2017-07-01 00:00:00" & datetime <"2021-07-06 23:59:00" )

## Regression lineaire avec le temps
fit_wink_p1 <- lm(data = p1wink, oxy_mll_eol_wink_detla ~ unclass(datetime) )
slope_wink_p1 <- fit_wink_p1$coefficients[[2]]
intercept_wink_p1 <- fit_wink_p1$coefficients[[1]]

lims_wink_p1 <- as.POSIXct(strptime(c("2017-07-01 00:00:00","2021-07-06 23:00:00"), format = "%Y-%m-%d %H:%M:%S"))    
wink_plot_p1 <-  ggplot(data=combined1, aes(x=datetime, y=oxy_mll_eol_wink_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_wink_p1, intercept =intercept_wink_p1), linetype = 1, col="red") + mytheme+labs(y="delta oxy EOL/Wink", x="") + scale_x_datetime(date_breaks="3 months",  labels=date_format("%b %Y"), limits =lims_wink_p1) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(wink_plot_p1)

#correction des valeurs EOL avec la regression
p1wink <- p1wink%>%
  dplyr::mutate(correction_oxy = slope_wink_p1 * unclass(datetime) + intercept_wink_p1,
                oxy_eol_corr = oxy_eol + correction_oxy )
# plot
 p1wink_corr_xts <-p1wink%>%
   dplyr::select(datetime, mean_oxy_mll_rhBplus_B, ptb_oxy_wink, oxy_eol, oxy_eol_corr)

p1wink_corr_xts <- as.xts(p1wink_corr_xts, order.by=p1wink_corr_xts$datetime)

dygraph(p1wink_corr_xts,  main = "p1, EOL - ptB+ - Winkler ptB", ylab = "") %>%
     dySeries("mean_oxy_mll_rhBplus_B", drawPoints = TRUE, pointSize=2, label = "oxy ptB+", color="blue")%>%
       dySeries("ptb_oxy_wink", drawPoints = TRUE, pointSize=2, label = "oxy winkler PTB", color="green")%>%
...
p2wink <- combined1%>%
  dplyr::select(datetime, mean_oxy_mll_rhBplus_B, oxy_eol, oxy_mll_eol_wink_detla ,ptb_oxy_wink )%>%
    #dplyr::filter(oxy_mll_eol_wink_detla > -0.3 & oxy_mll_eol_wink_detla < 0.3 )%>%
  dplyr::filter(datetime >"2021-07-13 00:00:00" & datetime <"2022-06-21 23:59:00" )

## Regression lineaire avec le temps
fit_wink_p2 <- lm(data = p2wink, oxy_mll_eol_wink_detla ~ unclass(datetime) )
slope_wink_p2 <- fit_wink_p2$coefficients[[2]]
intercept_wink_p2 <- fit_wink_p2$coefficients[[1]]

lims_wink_p2 <- as.POSIXct(strptime(c("2021-07-13 00:00:00","2022-06-21 23:59:00"), format = "%Y-%m-%d %H:%M:%S"))    
wink_plot_p2 <-  ggplot(data=combined1, aes(x=datetime, y=oxy_mll_eol_wink_detla), col="black") +  geom_point()+ geom_line()+ geom_abline(aes(slope= slope_wink_p2, intercept =intercept_wink_p2), linetype = 1, col="red") + mytheme+labs(y="delta oxy EOL/Wink", x="") + scale_x_datetime(date_breaks="1 month",  labels=date_format("%b %Y"), limits =lims_wink_p2) + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
print(wink_plot_p2)

#correction des valeurs EOL avec la regression
p2wink <- p2wink%>%
  dplyr::mutate(correction_oxy = slope_wink_p2 * unclass(datetime) + intercept_wink_p2,
                oxy_eol_corr = oxy_eol + correction_oxy )
# plot
 p2wink_corr_xts <-p2wink%>%
   dplyr::select(datetime, mean_oxy_mll_rhBplus_B, ptb_oxy_wink, oxy_eol, oxy_eol_corr)

p2wink_corr_xts <- as.xts(p2wink_corr_xts, order.by=p2wink_corr_xts$datetime)

dygraph(p2wink_corr_xts,  main = "p2, EOL corr - EOL - ptB+ - Winkler ptB", ylab = "") %>%
     dySeries("mean_oxy_mll_rhBplus_B", drawPoints = TRUE, pointSize=2, label = "oxy ptB+", color="blue")%>%
       dySeries("ptb_oxy_wink", drawPoints = TRUE, pointSize=2, label = "oxy winkler PTB", color="green")%>%
       dySeries("oxy_eol_corr", drawPoints = TRUE, pointSize=2, label = "oxy EOL corr", color="orange")%>%
         dySeries("oxy_eol", drawPoints = TRUE, pointSize=2, label = "oxy EOL", color="red")%>%
...
#correction des valeurs EOL avec la regression temps (low frequency)
combined1 <- combined1%>%
  dplyr::mutate(correction_oxy = case_when(datetime >"2017-07-11 00:00:00" & datetime < "2021-07-06 23:59:00" ~ slope_wink_p1 * unclass(datetime) + intercept_wink_p1, 
                                           datetime >"2021-07-13 00:00:00" & datetime <"2022-06-21 23:59:00"  ~ slope_wink_p2 * unclass(datetime) + intercept_wink_p2 , TRUE ~ 0),
                slope_oxy = case_when(datetime >"2017-07-11 00:00:00" & datetime < "2021-07-06 23:59:00" ~ slope_wink_p1,
                                      datetime >"2021-07-13 00:00:00" & datetime <"2022-06-21 23:59:00" ~ slope_wink_p2,
                                          TRUE ~ NA_real_),
                intercept_oxy = case_when(datetime >"2017-07-11 00:00:00" & datetime < "2021-07-06 23:59:00" ~  intercept_wink_p1, 
                                          datetime >"2021-07-13 00:00:00" & datetime <"2022-06-21 23:59:00" ~ intercept_wink_p2,
                                          TRUE ~ NA_real_),
                oxy_eol_corr = oxy_eol + correction_oxy )
### SAVE
saveRDS(combined1, file = paste0(path, "data/eol/combined_eol_rh_data.rds"))
...
## Plot LOW FREQUENCY 
eol_ptb_oxy_xts <- dplyr::select(combined1 , datetime, mean_oxy_mll_rhBplus_B, oxy_eol, ptb_oxy_wink, oxy_eol_corr)
#eol_ptb_oxy_xts <- dplyr::select(combined_1_B_Bplus_wink_eol, datetime, mean_oxy_mll_rhBplus, oxy_eol, ptb_oxy_wink, oxy_eol_corr)
eol_ptb_oxy_xts <- as.xts(eol_ptb_oxy_xts, order.by=eol_ptb_oxy_xts$datetime)

dygraph(eol_ptb_oxy_xts,  main = "Oxygen (ml/l) 1m: EOL - ptB+ - Winkler ptB (LOW FREQUENCY)", ylab = "") %>%
dySeries("mean_oxy_mll_rhBplus_B", drawPoints = TRUE, pointSize=4, label = "Oxy ptB+", color ="blue") %>%
dySeries("oxy_eol", drawPoints = TRUE, pointSize=2, label = "Oxy EOL", color ="red")%>%
dySeries("oxy_eol_corr", drawPoints = TRUE, pointSize=2, label = "Oxy EOL corr", color ="orange")%>%
dySeries("ptb_oxy_wink", drawPoints = TRUE, pointSize=2, label = "Oxy winkler PTB", color ="green")%>%
  dyShading(from =  oxy_periode[1], to= oxy_periode[2], color = "#C7C7C7") %>%
    dyShading(from =  oxy_periode[3], to= oxy_periode[4], color = "#C7C7C7") %>%
...
#add slope and intercpt from each LowFrequency data to eol data  HighFrequency.
eol <- eol%>%
  dplyr::mutate(correction_oxy = case_when(datetime >"2017-07-11 00:00:00" & datetime <"2021-05-25 23:59:00" ~ slope_wink_p1 * unclass(datetime) + intercept_wink_p1, 
                                           datetime >"2021-07-13 00:00:00" & datetime <"2022-06-21 23:59:00"  ~ slope_wink_p2 * unclass(datetime) + intercept_wink_p2, 
                                           TRUE ~ 0),
                oxy_eol_corr = oxy_eol + correction_oxy)
...
dygraph(eol_corr_xts, main = "EOL oxy (HIGH FREQUENCY)", ylab = "") %>%
...
## PLOT all (low freq)
eol_corr_xts <- dplyr::select(combined1, datetime, temp_eol, sal_eol_corr, oxy_eol_corr)
eol_corr_xts <- as.xts(eol_corr_xts, order.by=eol_corr_xts$datetime)

dygraph(eol_corr_xts, main = "EOL S T O data corrected LOW FREQUENCY", ylab = "") %>%
...
## PLOT all (low freq)
eol_corr_high_xts <- dplyr::select(eol, datetime, temp_eol, sal_eol_corr, oxy_eol_corr)
eol_corr_high_xts <- as.xts(eol_corr_high_xts, order.by=eol_corr_high_xts$datetime)

dygraph(eol_corr_high_xts, main = "EOL S T O data corrected HIGH FREQUENCY", ylab = "") %>%
    dySeries("oxy_eol_corr", drawPoints = TRUE, pointSize=2,label = "oxy")%>%
  dySeries(axis = "y2","sal_eol_corr", drawPoints = TRUE, pointSize=2, label = "Sal") %>%
    dyAxis(name = "y2", axisLineColor = "purple", axisLabelColor = "purple") %>% 
  dySeries("temp_eol", drawPoints = TRUE, pointSize=2, label = "Temp")%>%
...
# **Heatwaves**  {.tabset .tabset-pills}

```{r heatwave, echo=TRUE, warning = FALSE, message = FALSE, out.width='100%'}
library(heatwaveR)

hw <- eol%>%
  dplyr::select(datetime, temp_eol )

hwmean <- hw%>%
  dplyr::mutate(t = as.Date(datetime)) %>% 
  dplyr::group_by(t) %>% 
  dplyr::summarise(temp = mean(temp_eol, na.rm=TRUE)) %>% 
  heatwaveR::ts2clm(climatologyPeriod = c("2013-09-24", "2021-09-23"),maxPadLength = 5) %>% 
  heatwaveR::detect_event()

hwresults <- hwmean$event

saveRDS(hwmean, file = paste0(path, "data/eol/hwmean_robert.rds"))

# plot the greatest even
event_line(hwmean, spread = 180, metric = "intensity_cumulative", start_date = "1982-01-01", end_date = "2021-04-30")
#plot all the event
lolli_plot(hwmean, metric = "duration")

MHW_cat <- category(hwmean, S = FALSE, name = "EOL")

event_line(hwmean, spread = 200, start_date = "2010-11-01", end_date = "2021-04-30",metric = "intensity_max", category = TRUE)
```

```{r coldwave, echo=TRUE, warning = FALSE, message = FALSE, out.width='100%'}

cwmean <- hw%>%
  dplyr::mutate(t = as.Date(datetime)) %>% 
  dplyr::group_by(t) %>% 
  dplyr::summarise(temp = mean(temp_eol, na.rm=TRUE)) %>% 
  heatwaveR::ts2clm(climatologyPeriod = c("2013-09-24", "2021-09-23"),pctile = 10) %>% 
  heatwaveR::detect_event(coldSpells = TRUE)

cwresults <- cwmean$event

saveRDS(cwmean, file = paste0(path, "data/eol/cwmean_robert.rds"))

# plot the greatest even
event_line(cwmean, spread = 180, metric = "intensity_max", start_date = "1982-01-01", end_date = "2021-04-30")
#plot all the event
lolli_plot(cwmean, metric = "duration")

MCW_cat <- category(cwmean, S = FALSE, name = "EOL")

event_line(cwmean, spread = 200, start_date = "2010-11-01", end_date = "2021-04-30",metric = "intensity_max", category = TRUE)
```
